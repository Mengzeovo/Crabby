...
`,an=class extends E.AbstractInputSuggest{constructor(e,n,r){super(e,n),this.mode=r.mode,this.onChoose=r.onChoose,this.limit=12}async getSuggestions(e){return fi(this.app,e,this.mode)}renderSuggestion(e,n){let r=e.kind==="folder"&&!e.path.endsWith("/")?`${e.path}/`:e.path;n.createDiv({text:r}),n.createDiv({cls:"setting-item-description",text:e.kind==="folder"?"Vault \u6587\u4EF6\u5939":"Markdown \u6587\u4EF6"})}selectSuggestion(e,n){let r=this.mode==="markdownFile"&&e.kind==="folder"?`${e.path}/`:e.path;this.setValue(r),this.onChoose(r),this.close()}};async function fi(t,e,n){let r=new Map,s=a=>{let l=on(a.path);!l||!ln(l)||a.kind==="file"&&n==="folder"||a.kind==="file"&&!l.toLowerCase().endsWith(".md")||r.set(`${a.kind}:${l}`,{...a,path:l})};for(let a of t.vault.getAllLoadedFiles())vi(a,n,s);bi(n,s);for(let a of await yi(t,e))s(a);let i=on(e).toLowerCase();return Array.from(r.values()).map(a=>({candidate:a,score:ki(a,i)})).filter(a=>a.score>0||i.length===0).sort((a,l)=>l.score-a.score||xi(a.candidate,l.candidate)||a.candidate.path.localeCompare(l.candidate.path)).slice(0,12).map(a=>a.candidate)}function vi(t,e,n){if(t instanceof E.TFolder){t.path&&t.path!=="/"&&n({kind:"folder",path:t.path});return}e==="markdownFile"&&t instanceof E.TFile&&t.extension==="md"&&n({kind:"file",path:t.path})}function bi(t,e){if(e({kind:"folder",path:fe.rootPath}),t==="markdownFile")for(let n of Object.values(fe.templatePaths)){e({kind:"file",path:n});let r=vr(n);r&&e({kind:"folder",path:r})}}async function yi(t,e){let n=new Set(["",".crabby",".crabby/templates",".crabby/templates/diary"]),r=vr(e);r&&ln(r)&&n.add(r);let s=[];for(let i of n)if(!(i&&!ln(i)))try{let a=await t.vault.adapter.list(i);for(let l of a.folders)s.push({kind:"folder",path:l});for(let l of a.files)s.push({kind:"file",path:l})}catch{}return s}function ki(t,e){if(!e)return t.kind==="folder"?20:10;let n=t.path.toLowerCase(),r=n.split("/").pop()??n;return n===e?1e3:n.startsWith(e)?900:r.startsWith(e)?800:n.includes(`/${e}`)?700:n.includes(e)?500:0}function xi(t,e){return t.kind===e.kind?0:t.kind==="file"?-1:1}function vr(t){let e=on(t),n=e.lastIndexOf("/");return n<0?"":e.slice(0,n)}function on(t){return t.replace(/\\/g,"/").trim().replace(/^\/+/,"").split("/").filter(e=>e&&e!==".").join("/")}function ln(t){let e=t.replace(/\\/g,"/").trim();return e.length>0&&!e.startsWith("/")&&!e.startsWith("~")&&!/^[A-Za-z]:/.test(e)&&!e.split("/").some(n=>n==="..")}function rn(t,e,n=!1){let r=t.createEl("details");r.open=n,r.style.marginBottom="10px";let s=r.createEl("summary",{text:e});s.style.cursor="pointer",s.style.fontWeight="600",s.style.marginBottom="8px";let i=r.createDiv();return i.style.marginTop="10px",i}function Pi(t){return t.last_reload_ok===void 0||t.last_reload_ok===null?"\u5C1A\u672A\u6267\u884C":t.last_reload_ok?"\u6210\u529F":"\u5931\u8D25"}function wi(t){let e=Object.values(t.tools_by_server).reduce((s,i)=>s+i.length,0),n=t.connected_servers.length>0?t.connected_servers.join("\u3001"):"\u65E0",r=[`\u8FDE\u63A5\u72B6\u6001\uFF1A${t.connected_servers.length>0?`\u5DF2\u8FDE\u63A5 ${t.connected_servers.length} \u4E2A\u670D\u52A1`:"\u5F53\u524D\u6CA1\u6709\u5DF2\u8FDE\u63A5\u670D\u52A1"}`,`\u670D\u52A1\u5217\u8868\uFF1A${n}`,`\u5DE5\u5177\u603B\u6570\uFF1A${e}`,`\u6700\u8FD1\u91CD\u8F7D\uFF1A${Pi(t)}${t.last_reload_at?` \xB7 ${t.last_reload_at}`:""}`];if(t.vault_tools_enabled){let s=t.vault_tools_tools??[];r.push(`Vault \u5DE5\u5177\u96C6\uFF1A${s.length>0?`\u5DF2\u542F\u7528\uFF0C\u5DF2\u52A0\u8F7D ${s.length} \u4E2A\u5DE5\u5177\uFF08${s.join("\u3001")}\uFF09`:"\u5DF2\u542F\u7528\uFF0C\u5DE5\u5177\u76EE\u5F55\u4E3A\u7A7A"}`)}else r.push("Vault \u5DE5\u5177\u96C6\uFF1A\u672A\u542F\u7528");return t.last_reload_error&&r.push(`\u9519\u8BEF\u4FE1\u606F\uFF1A${t.last_reload_error}`),r.join(`
`)}var cn=class extends E.Modal{constructor(n,r){super(n);this.plugin=r}onOpen(){this.render()}onClose(){this.contentEl.empty()}async render(){let{contentEl:n}=this;n.empty(),n.createEl("h2",{text:"\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177"});let r=this.plugin.getCurrentVaultPath(),s=r?(0,sn.join)(r,".crabby","tools"):"",i=bt(this.plugin.settings,_t,!1),a=n.createEl("pre");Object.assign(a.style,{backgroundColor:"var(--background-secondary)",border:"1px solid var(--background-modifier-border)",borderRadius:"6px",padding:"10px 12px",whiteSpace:"pre-wrap",fontSize:"12px",lineHeight:"1.5",wordBreak:"break-word"});let l=(c="")=>{a.setText([`\u542F\u7528\u72B6\u6001\uFF1A${i?"\u5DF2\u542F\u7528":"\u672A\u542F\u7528"}`,`\u5DE5\u5177\u76EE\u5F55\uFF1A${s||"\u65E0\u6CD5\u68C0\u6D4B\u5F53\u524D Vault \u8DEF\u5F84"}`,c].filter(u=>u.trim()).join(`
`))},o=async()=>{l("\u6B63\u5728\u8BFB\u53D6\u540E\u7AEF\u5DE5\u5177\u72B6\u6001...");try{let c=new W(this.plugin.settings.backendUrl||Ee.backendUrl),u=await Zt(this.plugin.settings,c);if(!u.ok||!u.status){l(u.message);return}let p=u.status.vault_tools_tools??[];l(p.length>0?`\u5DF2\u52A0\u8F7D\u5DE5\u5177\uFF1A${p.join("\u3001")}`:"\u5DF2\u52A0\u8F7D\u5DE5\u5177\uFF1A\u65E0")}catch(c){let u=c instanceof Error?c.message:String(c);l(`\u8BFB\u53D6\u540E\u7AEF\u5DE5\u5177\u72B6\u6001\u5931\u8D25\uFF1A${u}`)}};new E.Setting(n).setName("\u521B\u5EFA\u5DE5\u5177\u76EE\u5F55").setDesc("\u521B\u5EFA .crabby/tools/\uFF0C\u7528\u4E8E\u653E\u7F6E\u81EA\u5B9A\u4E49 Python \u5DE5\u5177\u6587\u4EF6\u3002").addButton(c=>{c.setButtonText("\u521B\u5EFA\u76EE\u5F55"),c.setDisabled(!s),c.onClick(()=>{if(!s){new E.Notice("\u65E0\u6CD5\u68C0\u6D4B\u5F53\u524D Vault \u8DEF\u5F84\u3002");return}(0,ze.mkdirSync)(s,{recursive:!0}),new E.Notice("\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177\u76EE\u5F55\u5DF2\u521B\u5EFA\u3002"),l("\u5DE5\u5177\u76EE\u5F55\u5DF2\u521B\u5EFA\u3002")})}),new E.Setting(n).setName("\u521B\u5EFA\u793A\u4F8B\u5DE5\u5177").setDesc("\u5199\u5165 hello_tool.py \u793A\u4F8B\uFF1B\u5982\u679C\u6587\u4EF6\u5DF2\u5B58\u5728\uFF0C\u4E0D\u4F1A\u8986\u76D6\u3002").addButton(c=>{c.setButtonText("\u521B\u5EFA\u793A\u4F8B"),c.setDisabled(!s),c.onClick(()=>{if(!s){new E.Notice("\u65E0\u6CD5\u68C0\u6D4B\u5F53\u524D Vault \u8DEF\u5F84\u3002");return}(0,ze.mkdirSync)(s,{recursive:!0});let u=(0,sn.join)(s,"hello_tool.py");if((0,ze.existsSync)(u)){new E.Notice("hello_tool.py \u5DF2\u5B58\u5728\uFF0C\u672A\u8986\u76D6\u3002"),l(`\u793A\u4F8B\u5DE5\u5177\u5DF2\u5B58\u5728\uFF1A${u}`);return}(0,ze.writeFileSync)(u,hi,"utf8"),new E.Notice("\u793A\u4F8B\u5DE5\u5177\u5DF2\u521B\u5EFA\u3002"),l(`\u793A\u4F8B\u5DE5\u5177\u5DF2\u521B\u5EFA\uFF1A${u}`)})}),new E.Setting(n).setName("\u91CD\u8F7D\u5DE5\u5177").setDesc("\u4FDD\u5B58\u5F53\u524D\u542F\u7528\u72B6\u6001\uFF0C\u5E76\u8BA9\u540E\u7AEF\u91CD\u65B0\u52A0\u8F7D\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177\u3002").addButton(c=>{c.setButtonText("\u91CD\u8F7D"),c.setCta(),c.onClick(async()=>{c.setDisabled(!0);try{let u=new W(this.plugin.settings.backendUrl||Ee.backendUrl),p=await yt(this.plugin.settings,_t,i?"true":"false",u,"full");a.setText(p.message),new E.Notice(p.ok?"\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177\u5DF2\u91CD\u8F7D\u3002":p.message),p.ok&&await o()}catch(u){let p=u instanceof Error?u.message:String(u);a.setText(`\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177\u91CD\u8F7D\u5931\u8D25\uFF1A${p}`),new E.Notice(`\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177\u91CD\u8F7D\u5931\u8D25\uFF1A${p}`)}finally{c.setDisabled(!1)}})}),await o()}},Et=class extends E.PluginSettingTab{constructor(n,r){super(n,r);this.plugin=r;this.refreshToolRuntimeStatus=null}display(){let{containerEl:n}=this;n.empty(),n.createEl("h2",{text:"Crabby \u8BBE\u7F6E"}),this.renderRuntimeSection(n),this.renderMemorySection(n),this.renderToolsSection(n),this.renderDiarySection(n),this.renderMcpSection(n),this.renderLlmSection(n)}renderRuntimeSection(n){n.createEl("h3",{text:"\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F"});let r=this.plugin.runtimeManager;if(!r){n.createDiv().setText("\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u7BA1\u7406\u5668\u4E0D\u53EF\u7528\u3002");return}let s=this.plugin.settings.runtimeManifestUrl,i=n.createEl("pre");Object.assign(i.style,{backgroundColor:"var(--background-secondary)",border:"1px solid var(--background-modifier-border)",borderRadius:"6px",padding:"10px 12px",whiteSpace:"pre-wrap",fontSize:"12px",lineHeight:"1.5"});let a=0,l=async()=>{let o=++a,c=r.getStatus(),u=(f,x)=>{let d=x?.trim()||c.version;i.setText([`\u6A21\u5F0F\uFF1A${c.mode==="dev"?"\u5F00\u53D1\u7248":"\u6B63\u5F0F\u7248"}`,`\u540E\u7AEF\u7248\u672C\uFF1A${d}`,`\u540E\u7AEF\u7A0B\u5E8F\u5DF2\u5B89\u88C5\uFF1A${c.installed?"\u662F":"\u5426"}`,`\u540E\u7AEF\u8FDB\u7A0B\uFF1A${c.running?"\u8FD0\u884C\u4E2D":"\u672A\u8FD0\u884C"}`,`\u8FDE\u63A5\u72B6\u6001\uFF1A${f}`,`\u540E\u7AEF\u5730\u5740\uFF1A${c.backendUrl}`,`PID: ${c.pid??"-"}`,`Prompt config: ${c.promptsDir}`,`Persona config: ${c.personasDir}`,`.env \u6587\u4EF6\uFF1A${c.envPath}`,`MCP \u914D\u7F6E\uFF1A${c.mcpConfigPath}`,`\u6570\u636E\u76EE\u5F55\uFF1A${c.dataDir}`,`\u65E5\u5FD7\u76EE\u5F55\uFF1A${c.logsDir}`,`\u72B6\u6001\uFF1A${c.detail}`].join(`
`))};u("\u6B63\u5728\u68C0\u67E5...");let p=new W(c.backendUrl);try{let f=await p.getHealthStatus();o===a&&u(f.ok?"\u53EF\u8BBF\u95EE\uFF08/health \u6B63\u5E38\uFF09":"\u4E0D\u53EF\u8BBF\u95EE",f.version)}catch(f){if(o===a){let x=f instanceof Error?f.message:String(f);u(`\u4E0D\u53EF\u8BBF\u95EE\uFF1A${x}`)}}};new E.Setting(n).setName("\u540E\u7AEF\u7A0B\u5E8F\u4E0B\u8F7D\u6E05\u5355 URL").setDesc("\u7528\u4E8E\u5728\u7EBF\u5B89\u88C5\u6216\u66F4\u65B0\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u3002\u624B\u52A8\u5B89\u88C5\u5305\u901A\u5E38\u5DF2\u5185\u7F6E\uFF0C\u65E0\u9700\u586B\u5199\uFF1B\u5F00\u53D1\u7248\u4F1A\u4F18\u5148\u4F7F\u7528 .dev-runtime.json\u3002").addText(o=>{o.setPlaceholder("https://example.com/life-assistant/runtime-manifest.json").setValue(s).onChange(c=>{s=c.trim()}),o.inputEl.style.width="420px"}).addButton(o=>{o.setButtonText("\u4FDD\u5B58"),o.onClick(async()=>{this.plugin.settings.runtimeManifestUrl=s,await this.plugin.saveSettings(),new E.Notice("\u540E\u7AEF\u7A0B\u5E8F\u4E0B\u8F7D\u6E05\u5355 URL \u5DF2\u4FDD\u5B58\u3002")})}),new E.Setting(n).setName("\u5B89\u88C5/\u66F4\u65B0\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F").setDesc("\u4ECE\u4E0A\u9762\u7684\u6E05\u5355 URL \u4E0B\u8F7D\u5E76\u6821\u9A8C\u9002\u5408\u5F53\u524D\u5E73\u53F0\u7684\u540E\u7AEF\u7A0B\u5E8F\u3002\u624B\u52A8\u5B89\u88C5\u5305\u5DF2\u5185\u7F6E\u65F6\u4E0D\u9700\u8981\u70B9\u51FB\u3002").addButton(o=>{o.setButtonText("\u5B89\u88C5"),o.onClick(async()=>{o.setDisabled(!0);try{this.plugin.settings.runtimeManifestUrl=s,await this.plugin.saveSettings(),await r.installRuntime(s),new E.Notice("\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u5DF2\u5B89\u88C5\u3002")}catch(c){let u=c instanceof Error?c.message:String(c);new E.Notice(`\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u5B89\u88C5\u5931\u8D25\uFF1A${u}`)}finally{o.setDisabled(!1),await l()}})}),new E.Setting(n).setName("\u540E\u7AEF\u8FDB\u7A0B").setDesc("\u63A7\u5236\u7531\u5F53\u524D\u63D2\u4EF6\u7BA1\u7406\u7684\u672C\u5730\u540E\u7AEF\u8FDB\u7A0B\u3002").addButton(o=>{o.setButtonText("\u542F\u52A8"),o.onClick(async()=>{o.setDisabled(!0);try{await r.start(),await this.plugin.saveSettings()}catch(c){let u=c instanceof Error?c.message:String(c);new E.Notice(`\u540E\u7AEF\u542F\u52A8\u5931\u8D25\uFF1A${u}`)}finally{o.setDisabled(!1),await l()}})}).addButton(o=>{o.setButtonText("\u91CD\u542F"),o.onClick(async()=>{o.setDisabled(!0);try{await r.restart(),await this.plugin.saveSettings()}catch(c){let u=c instanceof Error?c.message:String(c);new E.Notice(`\u540E\u7AEF\u91CD\u542F\u5931\u8D25\uFF1A${u}`)}finally{o.setDisabled(!1),await l()}})}).addButton(o=>{o.setButtonText("\u505C\u6B62"),o.onClick(async()=>{o.setDisabled(!0);try{await r.stop()}catch(c){let u=c instanceof Error?c.message:String(c);new E.Notice(`\u540E\u7AEF\u505C\u6B62\u5931\u8D25\uFF1A${u}`)}finally{o.setDisabled(!1),await l()}})}).addButton(o=>{o.setButtonText("\u5237\u65B0"),o.onClick(()=>{l()})}),l()}renderMemorySection(n){n.createEl("h3",{text:"\u8BB0\u5FC6"});let r=he(this.plugin.settings),s=r.ok&&r.envPath?ne(r.envPath,tn):null,i=nr(this.plugin.settings,tn,nn),a=s??"",l=n.createDiv();Object.assign(l.style,{fontSize:"12px",color:"var(--text-muted)",marginBottom:"10px",whiteSpace:"pre-wrap",lineHeight:"1.5"}),l.setText(r.ok?`\u5F53\u524D\u751F\u6548\uFF1A${i}\uFF1B\u7559\u7A7A\u6062\u590D\u9ED8\u8BA4 ${nn}\uFF0C0 \u8868\u793A\u5173\u95ED\u3002`:r.message),new E.Setting(n).setName("\u81EA\u52A8\u8BB0\u5FC6\u6C89\u6DC0\u95F4\u9694").setDesc("\u6309\u5BF9\u8BDD\u8F6E\u6570\u89E6\u53D1\u540E\u53F0\u8BB0\u5FC6\u6C89\u6DC0\uFF1B\u8BF7\u8F93\u5165\u975E\u8D1F\u6574\u6570\uFF0C0 \u8868\u793A\u5173\u95ED\u3002").addText(o=>{o.setPlaceholder(String(nn)).setValue(a).onChange(c=>{a=c.trim()}),o.inputEl.type="number",o.inputEl.min="0",o.inputEl.step="1",o.inputEl.style.width="120px"}).addButton(o=>{o.setButtonText("\u4FDD\u5B58"),o.onClick(async()=>{let c=tr(a);if(!c.ok){l.setText(c.message),new E.Notice(c.message);return}o.setDisabled(!0);try{let u=new W(this.plugin.settings.backendUrl||Ee.backendUrl),p=await yt(this.plugin.settings,tn,c.envValue,u,"settings");l.setText(p.message),new E.Notice(p.ok?"\u81EA\u52A8\u8BB0\u5FC6\u6C89\u6DC0\u914D\u7F6E\u5DF2\u4FDD\u5B58\u3002":p.message)}catch(u){let p=u instanceof Error?u.message:String(u);l.setText(`\u81EA\u52A8\u8BB0\u5FC6\u6C89\u6DC0\u914D\u7F6E\u4FDD\u5B58\u5931\u8D25\uFF1A${p}`),new E.Notice(`\u81EA\u52A8\u8BB0\u5FC6\u6C89\u6DC0\u914D\u7F6E\u4FDD\u5B58\u5931\u8D25\uFF1A${p}`)}finally{o.setDisabled(!1)}})})}renderToolsSection(n){n.createEl("h3",{text:"\u5DE5\u5177\u4E0E\u6743\u9650"});let r=bt(this.plugin.settings,fr,!0),s=bt(this.plugin.settings,_t,!1),i=n.createDiv();Object.assign(i.style,{fontSize:"12px",color:"var(--text-muted)",marginBottom:"10px",minHeight:"18px",whiteSpace:"pre-wrap",lineHeight:"1.5"}),i.setText("Bash \u5DE5\u5177\u548C\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177\u7684\u542F\u7528\u72B6\u6001\u4FDD\u5B58\u5728\u540E\u7AEF .env\u3002");let a=async(l,o,c)=>{try{let u=new W(this.plugin.settings.backendUrl||Ee.backendUrl),p=await yt(this.plugin.settings,l,o?"true":"false",u,c);i.setText(p.message),new E.Notice(p.ok?"\u5DE5\u5177\u914D\u7F6E\u5DF2\u4FDD\u5B58\u3002":p.message),p.ok&&this.refreshToolRuntimeStatus?.()}catch(u){let p=u instanceof Error?u.message:String(u);i.setText(`\u5DE5\u5177\u914D\u7F6E\u4FDD\u5B58\u5931\u8D25\uFF1A${p}`),new E.Notice(`\u5DE5\u5177\u914D\u7F6E\u4FDD\u5B58\u5931\u8D25\uFF1A${p}`)}};new E.Setting(n).setName("Bash \u5DE5\u5177").setDesc("\u5141\u8BB8\u6A21\u578B\u6267\u884C\u672C\u5730\u975E\u4EA4\u4E92\u5F0F shell \u547D\u4EE4\u3002\u5173\u95ED\u540E\u4F1A\u4ECE\u540E\u7AEF\u5DE5\u5177\u5217\u8868\u79FB\u9664 bash\u3002").addToggle(l=>{l.setValue(r).onChange(async o=>{r=o,await a(fr,o,"settings")})}),new E.Setting(n).setName("\u7528\u6237\u81EA\u5B9A\u4E49\u5DE5\u5177").setDesc("\u542F\u7528 Vault \u5185 .crabby/tools/ \u4E0B\u7684\u81EA\u5B9A\u4E49 Python \u5DE5\u5177\u3002").addToggle(l=>{l.setValue(s).onChange(async o=>{s=o,await a(_t,o,"full")})}).addButton(l=>{l.setButtonText("\u7BA1\u7406"),l.onClick(()=>{new cn(this.app,this.plugin).open()})}),this.refreshToolRuntimeStatus=this.renderToolRuntimeStatus(n)}renderToolRuntimeStatus(n){n.createEl("h4",{text:"\u8FD0\u884C\u4E2D\u7684\u5DE5\u5177"});let r=()=>this.plugin.settings.backendUrl||Ee.backendUrl,s=n.createDiv({cls:"mcp-runtime-summary"});Object.assign(s.style,{backgroundColor:"var(--background-secondary)",border:"1px solid var(--background-modifier-border)",borderRadius:"8px",padding:"12px 14px",marginBottom:"10px",fontSize:"12px",lineHeight:"1.6",whiteSpace:"pre-wrap",color:"var(--text-normal)"}),s.setText("\u6B63\u5728\u8BFB\u53D6\u5DE5\u5177\u8FD0\u884C\u72B6\u6001...");let i=n.createDiv({cls:"mcp-status-bar"});i.style.fontSize="12px",i.style.color="var(--text-muted)",i.style.marginBottom="10px",i.style.minHeight="18px";let l=rn(n,"\u67E5\u770B MCP \u670D\u52A1\u4E0E Vault \u5DE5\u5177\u8BE6\u60C5").createEl("pre",{cls:"mcp-runtime-status"});Object.assign(l.style,{backgroundColor:"var(--background-secondary)",border:"1px solid var(--background-modifier-border)",borderRadius:"6px",padding:"10px 12px",marginBottom:"0",fontSize:"12px",fontFamily:"var(--font-monospace)",whiteSpace:"pre-wrap",wordBreak:"break-word",lineHeight:"1.5",color:"var(--text-normal)"}),l.setText("\u6B63\u5728\u8BFB\u53D6\u5DE5\u5177\u8FD0\u884C\u72B6\u6001...");let o=async()=>{let c="\u6B63\u5728\u8BFB\u53D6\u5DE5\u5177\u8FD0\u884C\u72B6\u6001...";s.setText(c),l.setText(c),i.setText("");try{let u=new W(r()),p=await Zt(this.plugin.settings,u);p.ok&&p.status?(s.setText(wi(p.status)),l.setText(dr(p.status)),i.setText("\u5DE5\u5177\u8FD0\u884C\u72B6\u6001\u5DF2\u5237\u65B0\u3002")):(s.setText(p.message),l.setText(p.message),i.setText(p.message))}catch(u){let f=`\u8BFB\u53D6\u5DE5\u5177\u8FD0\u884C\u72B6\u6001\u5931\u8D25\uFF1A${u instanceof Error?u.message:String(u)}`;s.setText(f),l.setText(f),i.setText(f)}};return new E.Setting(n).setName("\u5DE5\u5177\u8FD0\u884C\u72B6\u6001").setDesc("\u91CD\u65B0\u8BFB\u53D6\u540E\u7AEF\u5F53\u524D\u53EF\u7528\u7684 MCP \u670D\u52A1\u3001MCP \u5DE5\u5177\u548C Vault \u81EA\u5B9A\u4E49\u5DE5\u5177\u3002").addButton(c=>{c.setButtonText("\u5237\u65B0"),c.onClick(()=>{o()})}),o(),()=>{o()}}renderDiarySection(n){n.createEl("h3",{text:"Diary / Journal"});let r=n.createDiv();Object.assign(r.style,{fontSize:"12px",color:"var(--text-muted)",marginBottom:"10px",whiteSpace:"pre-wrap",lineHeight:"1.5"});let s={rootPath:this.plugin.settings.diary.rootPath,templatePaths:{...this.plugin.settings.diary.templatePaths}},i=async()=>{let o;try{o=He(s)}catch(u){let p=u instanceof Error?u.message:String(u);r.setText(`Diary \u914D\u7F6E\u65E0\u6548\uFF1A${p}`),new E.Notice(`Diary \u914D\u7F6E\u65E0\u6548\uFF1A${p}`);return}this.plugin.settings.diary=o,await this.plugin.saveSettings();let c=this.plugin.runtimeManager?.syncDiaryConfig();if(!c){r.setText("Diary \u914D\u7F6E\u5DF2\u4FDD\u5B58\uFF1B\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u521D\u59CB\u5316\u540E\u4F1A\u540C\u6B65\u3002");return}if(c.ok===!1){r.setText(`Diary \u914D\u7F6E\u5DF2\u4FDD\u5B58\uFF0C\u4F46\u540C\u6B65\u5931\u8D25\uFF1A${c.message}`);return}r.setText("Diary \u914D\u7F6E\u5DF2\u4FDD\u5B58\uFF0C\u5E76\u540C\u6B65\u5230 .crabby/config/diary.json\u3002")},a=(o,c,u,p,f)=>{new E.Setting(n).setName(o).addText(x=>{x.setPlaceholder(u).setValue(c).onChange(d=>{f(d.trim())}),x.inputEl.style.width="420px",new an(this.app,x.inputEl,{mode:p,onChoose:d=>{f(d.trim())}})})};a("\u65E5\u8BB0\u6839\u76EE\u5F55",s.rootPath,"Journal/","folder",o=>{s.rootPath=o||"Journal"}),a("\u65E5\u8BB0\u6A21\u677F\uFF08\u65E5\uFF09",s.templatePaths.daily,".crabby/templates/diary/daily.md","markdownFile",o=>{s.templatePaths.daily=o}),a("\u65E5\u8BB0\u6A21\u677F\uFF08\u5468\uFF09",s.templatePaths.weekly,".crabby/templates/diary/weekly.md","markdownFile",o=>{s.templatePaths.weekly=o}),a("\u65E5\u8BB0\u6A21\u677F\uFF08\u6708\uFF09",s.templatePaths.monthly,".crabby/templates/diary/monthly.md","markdownFile",o=>{s.templatePaths.monthly=o}),a("\u65E5\u8BB0\u6A21\u677F\uFF08\u5B63\uFF09",s.templatePaths.quarterly,".crabby/templates/diary/quarterly.md","markdownFile",o=>{s.templatePaths.quarterly=o}),a("\u65E5\u8BB0\u6A21\u677F\uFF08\u5E74\uFF09",s.templatePaths.yearly,".crabby/templates/diary/yearly.md","markdownFile",o=>{s.templatePaths.yearly=o}),new E.Setting(n).setName("\u4FDD\u5B58 Diary \u914D\u7F6E").setDesc("\u628A\u4E0A\u9762\u7684\u6839\u76EE\u5F55\u548C\u6A21\u677F\u8DEF\u5F84\u5199\u5165 .crabby/config/diary.json\u3002").addButton(o=>{o.setButtonText("\u4FDD\u5B58"),o.onClick(()=>{i()})});let l=this.plugin.runtimeManager?.getLayout().configDir?`${this.plugin.runtimeManager.getLayout().configDir}/diary.json`:".crabby/config/diary.json";r.setText(`\u914D\u7F6E\u6587\u4EF6\uFF1A${l}`)}renderMcpSection(n){n.createEl("h3",{text:"MCP \u670D\u52A1\u914D\u7F6E"});let r=this.plugin.settings.backendMcpConfigPath,s=()=>this.plugin.settings.backendUrl||Ee.backendUrl,i=()=>({...this.plugin.settings,backendMcpConfigPath:r}),a=n.createDiv({cls:"mcp-config-hint"});Object.assign(a.style,{fontSize:"12px",color:"var(--text-muted)",marginBottom:"10px",lineHeight:"1.5",whiteSpace:"pre-wrap",wordBreak:"break-word"});let l=n.createDiv({cls:"mcp-config-status-bar"});l.style.fontSize="12px",l.style.color="var(--text-muted)",l.style.marginBottom="10px",l.style.minHeight="18px";let o=()=>{let d=at(i());if(!d.ok||!d.configPath){a.setText(d.message);return}let P=d.derivedFromBackendEnvPath?"\u81EA\u52A8\u4ECE\u63D2\u4EF6\u914D\u7F6E\u76EE\u5F55\u63A8\u5BFC":"\u624B\u52A8\u8986\u76D6\u8DEF\u5F84",C=d.examplePath?`
\u6A21\u677F\u6587\u4EF6\uFF1A${d.examplePath}`:"";a.setText(`\u5F53\u524D MCP \u914D\u7F6E\u6587\u4EF6\uFF1A${d.configPath}
\u8DEF\u5F84\u6765\u6E90\uFF1A${P}${C}`)},c=async()=>{this.plugin.settings.backendMcpConfigPath=r,await this.plugin.saveSettings()},u=rn(n,"\u9AD8\u7EA7\u8DEF\u5F84\u8986\u76D6",!!r);new E.Setting(u).setName("MCP \u914D\u7F6E\u6587\u4EF6\u8DEF\u5F84").setDesc("\u4E00\u822C\u4E0D\u9700\u8981\u8BBE\u7F6E\u3002\u4EC5\u5728 mcp_servers.json \u4E0D\u5728\u9ED8\u8BA4\u4F4D\u7F6E\uFF08<vault>/.crabby/config/server/data/\uFF09\u65F6\u624B\u52A8\u586B\u5199\u3002").addText(d=>{d.setPlaceholder("D:\\path\\to\\Crabby\\server\\data\\mcp_servers.json").setValue(r).onChange(P=>{r=P.trim(),o()}),d.inputEl.style.width="320px"});let p=rn(n,"\u7F16\u8F91 mcp_servers.json"),f=p.createEl("textarea",{cls:"mcp-config-editor"});Object.assign(f.style,{width:"100%",minHeight:"280px",boxSizing:"border-box",padding:"10px 12px",marginBottom:"10px",borderRadius:"6px",border:"1px solid var(--background-modifier-border)",backgroundColor:"var(--background-primary)",color:"var(--text-normal)",fontFamily:"var(--font-monospace)",fontSize:"12px",lineHeight:"1.5",resize:"vertical"}),f.placeholder=`{
  "mcpServers": {}
}
`;let x=()=>{let d=or(i());d.ok&&(f.value=d.text??""),l.setText(d.message),o()};new E.Setting(p).setName("\u4ECE\u6587\u4EF6\u8F7D\u5165").setDesc("\u628A\u78C1\u76D8\u4E0A\u7684 mcp_servers.json \u91CD\u65B0\u8F7D\u5165\u5230\u7F16\u8F91\u5668\u3002").addButton(d=>{d.setButtonText("\u8F7D\u5165"),d.onClick(()=>{x()})}),new E.Setting(p).setName("\u4ECE\u6A21\u677F\u521B\u5EFA").setDesc("\u5F53\u771F\u5B9E\u914D\u7F6E\u6587\u4EF6\u4E0D\u5B58\u5728\u65F6\uFF0C\u6839\u636E mcp_servers.example.json \u521B\u5EFA\u3002").addButton(d=>{d.setButtonText("\u521B\u5EFA"),d.onClick(async()=>{await c();let P=lr(this.plugin.settings);P.ok?(f.value=P.text??"",l.setText(P.message),new E.Notice("\u5DF2\u6839\u636E\u6A21\u677F\u521B\u5EFA MCP \u914D\u7F6E\u6587\u4EF6\u3002")):(l.setText(P.message),new E.Notice(`\u521B\u5EFA\u5931\u8D25\uFF1A${P.message}`)),o()})}),new E.Setting(p).setName("\u672C\u5730\u6821\u9A8C").setDesc("\u53EA\u6821\u9A8C JSON \u8BED\u6CD5\u548C MCP \u914D\u7F6E\u7ED3\u6784\uFF0C\u4E0D\u4F1A\u5199\u5165\u540E\u7AEF\u3002").addButton(d=>{d.setButtonText("\u6821\u9A8C"),d.onClick(()=>{let P=Jt(f.value);l.setText(P.message),P.ok?new E.Notice("MCP \u914D\u7F6E\u6821\u9A8C\u901A\u8FC7\u3002"):new E.Notice(`\u6821\u9A8C\u5931\u8D25\uFF1A${P.message}`)})}),new E.Setting(p).setName("\u4FDD\u5B58\u914D\u7F6E").setDesc("\u628A\u7F16\u8F91\u5668\u5185\u5BB9\u5199\u5165 mcp_servers.json\uFF08\u9700\u8981\u5148\u5728\u9AD8\u7EA7\u8DEF\u5F84\u8986\u76D6\u91CC\u914D\u7F6E\u8DEF\u5F84\uFF0C\u6216\u914D\u7F6E\u597D .env\uFF09\u3002").addButton(d=>{d.setButtonText("\u4FDD\u5B58"),d.onClick(async()=>{await c();let P=Xt(this.plugin.settings,f.value);l.setText(P.message),P.ok?new E.Notice("MCP \u914D\u7F6E\u5DF2\u4FDD\u5B58\u3002"):new E.Notice(`\u4FDD\u5B58\u5931\u8D25\uFF1A${P.message}`),o()})}).addButton(d=>{d.setButtonText("\u4FDD\u5B58\u5E76\u91CD\u8F7D"),d.setCta(),d.onClick(async()=>{await c();let P=Xt(this.plugin.settings,f.value);if(!P.ok){l.setText(P.message),new E.Notice(`\u4FDD\u5B58\u5931\u8D25\uFF1A${P.message}`),o();return}l.setText(`${P.message} \u6B63\u5728\u91CD\u8F7D\u540E\u7AEF...`);let C=new W(s()),y=await cr(this.plugin.settings,C);l.setText(y.message),y.ok?(new E.Notice("MCP \u914D\u7F6E\u5DF2\u4FDD\u5B58\uFF0C\u5E76\u5B8C\u6210\u540E\u7AEF\u91CD\u8F7D\u3002"),this.refreshToolRuntimeStatus?.()):new E.Notice(`\u91CD\u8F7D\u5931\u8D25\uFF1A${y.message}`),o()})}),o(),x()}renderLlmSection(n){n.createEl("h3",{text:"LLM \u914D\u7F6E"});let r=he(this.plugin.settings),s=n.createDiv({cls:"llm-config-hint"});s.style.fontSize="12px",s.style.marginBottom="10px",s.style.wordBreak="break-word",r.ok&&r.envPath?(s.style.color="var(--text-muted)",s.setText(`\u5F53\u524D\u751F\u6548\u914D\u7F6E\u6587\u4EF6\uFF1A${r.envPath}`)):(s.style.color="var(--text-accent)",s.style.fontWeight="600",s.setText(r.message));let i=n.createDiv({cls:"llm-status-bar"});i.style.fontSize="12px",i.style.color="var(--text-muted)",i.style.marginBottom="10px",i.style.minHeight="18px",i.style.wordBreak="break-word";let a=n.createDiv({cls:"llm-profile-list"});a.style.marginBottom="4px";let l=()=>this.plugin.settings.backendUrl||Ee.backendUrl,o=async()=>{i.setText("\u6B63\u5728\u4ECE\u540E\u7AEF\u8BFB\u53D6 LLM \u914D\u7F6E...");try{let d=await this.plugin.syncLlmProfilesFromBackend({migrateLocalProfiles:!1});i.setText(d.message),d.ok&&(x(),c())}catch(d){let P=d instanceof Error?d.message:String(d);i.setText(`\u8BFB\u53D6\u540E\u7AEF LLM \u914D\u7F6E\u5931\u8D25\uFF1A${P}`)}},c=()=>{let d=this.plugin.settings.llmProfiles.find(C=>C.id===this.plugin.settings.activeProfileId&&!ve(C)),P=this.plugin.settings.llmProfiles.find(C=>C.id===this.plugin.settings.activeProfileId&&ve(C));d?i.setText(`\u5F53\u524D\u542F\u7528\uFF1A${d.name}\uFF08${d.provider} / ${d.model}\uFF09`):P?i.setText("\u5F53\u524D\u6B63\u5728\u7F16\u8F91\u672A\u4FDD\u5B58\u8349\u7A3F\u3002\u4FDD\u5B58\u540E\u624D\u80FD\u542F\u7528\u3002"):this.plugin.settings.llmProfiles.length>0?i.setText("\u5F53\u524D\u8FD8\u6CA1\u6709\u9009\u4E2D\u7684\u914D\u7F6E\u3002"):i.setText("\u5F53\u524D\u8FD8\u6CA1\u6709\u521B\u5EFA\u4EFB\u4F55 LLM \u914D\u7F6E\u3002")},u=async d=>{i.setText(`\u6B63\u5728\u5E94\u7528 ${d.name} ...`);let P=new W(l());try{let C=await Ue(this.plugin.settings,d,P,!0);return i.setText(C.message),C.ok?(await this.plugin.saveSettings(),x(),new E.Notice(`\u5DF2\u5207\u6362\u5230 ${d.name}\u3002`),!0):(x(),new E.Notice(`\u5207\u6362\u5931\u8D25\uFF1A${C.message}`),!1)}catch(C){let y=C instanceof Error?C.message:String(C);return i.setText(`\u5207\u6362\u5931\u8D25\uFF1A${y}`),x(),new E.Notice(`\u5207\u6362\u5931\u8D25\uFF1A${y}`),!1}},p=async d=>{let P=d.id===this.plugin.settings.activeProfileId;i.setText(`\u6B63\u5728\u4FDD\u5B58 ${d.name} \u5230\u540E\u7AEF...`);let C=new W(l());try{let y=await Ue(this.plugin.settings,d,C,P);i.setText(y.message),y.ok?(await this.plugin.saveSettings(),x(),c(),new E.Notice(`\u5DF2\u4FDD\u5B58 ${d.name}\u3002`)):new E.Notice(`\u4FDD\u5B58\u5931\u8D25\uFF1A${y.message}`)}catch(y){let _=y instanceof Error?y.message:String(y);i.setText(`\u4FDD\u5B58\u5931\u8D25\uFF1A${_}`),new E.Notice(`\u4FDD\u5B58\u5931\u8D25\uFF1A${_}`)}},f=async()=>{let d=this.plugin.settings.llmProfiles.find(M=>M.id===this.plugin.settings.activeProfileId&&!ve(M)),P=he(this.plugin.settings);if(!P.ok||!P.envPath){i.setText(P.message);return}let C=ne(P.envPath,"CRABBY_ADMIN_TOKEN")?.trim();if(!C){i.setText(`\u65E0\u6CD5\u6D4B\u8BD5\u5F53\u524D Profile\uFF1A${P.envPath} \u7F3A\u5C11 CRABBY_ADMIN_TOKEN\u3002`);return}let y=d?`${d.name}\uFF08${d.provider} / ${d.model}\uFF09`:"\u540E\u7AEF\u5F53\u524D\u5DF2\u751F\u6548\u914D\u7F6E";i.setText(`\u6B63\u5728\u6D4B\u8BD5\u5F53\u524D Profile\uFF1A${y}...`);let A=await new W(l()).testCurrentProfile(C);if(!A.ok||!A.data){let M=A.status===null?"\u540E\u7AEF\u5F53\u524D\u4E0D\u53EF\u8BBF\u95EE\u3002":A.detail||`HTTP ${A.status}`;i.setText(`\u6D4B\u8BD5\u5931\u8D25\uFF1A${M}`),new E.Notice(`\u6D4B\u8BD5\u5931\u8D25\uFF1A${M}`);return}i.setText(A.data.message),new E.Notice(A.data.ok?A.data.message:`\u6D4B\u8BD5\u672A\u901A\u8FC7\uFF1A${A.data.message}`)},x=()=>{if(a.empty(),this.plugin.settings.llmProfiles.length===0){let d=a.createDiv();d.setText("\u8FD8\u6CA1\u6709\u914D\u7F6E\u3002\u70B9\u51FB\u201C\u6DFB\u52A0\u914D\u7F6E\u201D\u521B\u5EFA\u4E00\u4E2A\u65B0\u7684 LLM \u914D\u7F6E\u3002"),d.style.color="var(--text-muted)",d.style.fontStyle="italic",d.style.padding="8px 0";return}this.plugin.settings.llmProfiles.forEach((d,P)=>{en(d);let C=ve(d),y=d.id===this.plugin.settings.activeProfileId&&!C,_=a.createDiv({cls:"llm-profile-card"});Object.assign(_.style,{border:`1px solid ${y?"var(--interactive-accent)":"var(--background-modifier-border)"}`,borderRadius:"8px",padding:"12px 16px",marginBottom:"10px",backgroundColor:y?"var(--background-secondary-alt)":"var(--background-secondary)",transition:"border-color 0.15s, background-color 0.15s"});let A=_.createDiv();Object.assign(A.style,{display:"flex",alignItems:"center",gap:"8px",marginBottom:"10px",flexWrap:"wrap"});let M=A.createSpan();M.style.fontSize="16px",M.style.cursor="pointer",M.title=y?"\u8FD9\u4E2A\u914D\u7F6E\u5F53\u524D\u5DF2\u542F\u7528\u3002":C?"\u70B9\u51FB\u4FDD\u5B58\u5E76\u542F\u7528\u8FD9\u4E2A\u8349\u7A3F\u914D\u7F6E\u3002":"\u70B9\u51FB\u542F\u7528\u8FD9\u4E2A\u914D\u7F6E\uFF0C\u5E76\u70ED\u91CD\u8F7D\u540E\u7AEF\u3002",M.setText(y?"\u25CF":"\u25CB"),M.addEventListener("click",async()=>{await u(d)});let z=A.createEl("strong"),U=()=>d.name||`\u914D\u7F6E ${P+1}`;z.setText(U()),z.style.flex="1",z.style.minWidth="0",z.style.fontSize="14px",z.style.overflow="hidden",z.style.textOverflow="ellipsis",z.style.whiteSpace="nowrap";let j=Object.fromEntries(pt.map(m=>[m,ue(m).badge])),X=A.createSpan();if(Object.assign(X.style,{fontSize:"11px",padding:"2px 8px",borderRadius:"12px",backgroundColor:j[d.provider],color:"#fff",fontWeight:"600",letterSpacing:"0.03em"}),(()=>{let m=String(d.provider||"");X.setText(m.toUpperCase()||"UNKNOWN"),X.style.backgroundColor=j[m]??"var(--text-muted)"})(),C){let m=A.createSpan();Object.assign(m.style,{fontSize:"11px",padding:"2px 8px",borderRadius:"12px",backgroundColor:"var(--background-modifier-border)",color:"var(--text-muted)",fontWeight:"600"}),m.setText("\u8349\u7A3F")}let S=A.createEl("button");S.setText("\u4FDD\u5B58"),S.title=C?"\u628A\u8FD9\u4E2A\u8349\u7A3F\u914D\u7F6E\u4FDD\u5B58\u5230\u540E\u7AEF .env\u3002":y?"\u4FDD\u5B58\u8FD9\u4E2A\u914D\u7F6E\uFF0C\u5E76\u7ACB\u5373\u5E94\u7528\u5230\u540E\u7AEF\u3002":"\u628A\u8FD9\u4E2A\u914D\u7F6E\u4FDD\u5B58\u5230\u540E\u7AEF\u3002",S.addEventListener("click",()=>{p(d)});let D=A.createEl("button");D.setText("\u5220\u9664"),D.title="\u5220\u9664\u8FD9\u4E2A\u914D\u7F6E\u3002",D.addEventListener("click",async()=>{let m=async()=>{this.plugin.settings.llmProfiles=this.plugin.settings.llmProfiles.filter(k=>k.id!==d.id),this.plugin.settings.activeProfileId===d.id&&(this.plugin.settings.activeProfileId=this.plugin.settings.llmProfiles[0]?.id??""),await this.plugin.saveSettings(),x(),c()};i.setText(`\u6B63\u5728\u5220\u9664 ${d.name}...`);let g=new W(l()),b=await xt(this.plugin.settings,d.id,g);if(i.setText(b.message),!b.ok){if(b.message.includes("Profile not found")){await m(),new E.Notice(`\u5DF2\u5220\u9664\u672C\u5730\u8349\u7A3F ${d.name}\u3002`);return}new E.Notice(`\u5220\u9664\u5931\u8D25\uFF1A${b.message}`);return}await m(),new E.Notice(`\u5DF2\u5220\u9664 ${d.name}\u3002`)});{let{activePreset:m,capabilities:g}=pi(d),b=O=>{Object.assign(O.style,{display:"grid",gridTemplateColumns:"80px minmax(0, 1fr)",alignItems:"center",gap:"8px",marginBottom:"6px"})},k=O=>{Object.assign(O.style,{fontSize:"12px",color:"var(--text-muted)",textAlign:"right"})},h=O=>{Object.assign(O.style,{width:"100%",boxSizing:"border-box",fontSize:"13px",padding:"4px 8px",borderRadius:"4px",border:"1px solid var(--background-modifier-border)",backgroundColor:"var(--background-primary)",color:"var(--text-normal)"})},w=(O,N,se,ie,$e,je="text")=>{let Ke=O.createDiv();b(Ke);let Me=Ke.createEl("label");Me.setText(N),k(Me);let De=Ke.createEl("input");return De.type=je,De.placeholder=ie,De.value=se,h(De),De.addEventListener("input",async()=>{await $e(De.value),c()}),De},$=(O,N,se,ie)=>{let $e=O.createDiv();b($e);let je=$e.createEl("label");je.setText(N),k(je);let Me=$e.createDiv().createEl("input");Me.type="checkbox",Me.checked=se,Me.addEventListener("change",async()=>{await ie(Me.checked),c()})};w(_,"Name",d.name,"Daily driver",async O=>{d.name=O,await this.plugin.saveSettings(),z.setText(U())});let I=_.createDiv();b(I);let V=I.createEl("label");V.setText("Provider"),k(V);let J=I.createEl("select");h(J),pt.forEach(O=>{let N=J.createEl("option");N.value=O,N.setText(ue(O).label)}),J.value=d.provider,J.addEventListener("change",async()=>{d.provider=J.value;let O=ue(d.provider),N=Xn(d.provider);d.model=N||d.model,d.baseUrl=O.defaultBaseUrl,en(d),O.capabilities.thinking||(d.thinkingMode=""),O.capabilities.thinkingBudget||(d.thinkingBudgetTokens="1024"),O.capabilities.reasoningEffort||(d.thinkingEffort=""),O.capabilities.reasoningSplit||(d.reasoningSplit=!1),await this.plugin.saveSettings(),x(),c()});let Z=_.createEl("datalist");Z.id=`llm-models-${d.id}`,m.models.forEach(O=>{let N=Z.createEl("option");N.value=O.id,N.label=O.label});let te=w(_,"Model",d.model,"Select or type a model id",async O=>{d.model=O.trim(),en(d),await this.plugin.saveSettings()});if(te.setAttribute("list",Z.id),te.addEventListener("change",()=>{x(),c()}),g.baseUrl&&w(_,"Base URL",d.baseUrl,m.defaultBaseUrl,async O=>{d.baseUrl=O.trim(),await this.plugin.saveSettings()}),g.apiKey&&w(_,"API Key",d.apiKey,m.apiKeyEnv||"LLM_API_KEY",async O=>{d.apiKey=O.trim(),await this.plugin.saveSettings()},"password"),g.vision||g.thinking||g.thinkingBudget||g.reasoningEffort||g.reasoningSplit){let O=_.createEl("details");O.style.marginTop="8px";let N=O.createEl("summary");N.setText("Advanced"),N.style.cursor="pointer",N.style.fontSize="12px",N.style.color="var(--text-muted)";let se=O.createDiv();se.style.marginTop="8px",g.vision&&$(se,"Vision",!!d.supportsVision,async ie=>{d.supportsVision=ie,await this.plugin.saveSettings()}),g.thinking&&$(se,"Thinking",d.thinkingMode.trim().toLowerCase()==="enabled",async ie=>{d.thinkingMode=ie?"enabled":"",await this.plugin.saveSettings()}),g.thinkingBudget&&w(se,"Budget",d.thinkingBudgetTokens,"1024",async ie=>{d.thinkingBudgetTokens=ie.trim(),await this.plugin.saveSettings()}),g.reasoningEffort&&w(se,"Effort",d.thinkingEffort,Jn(d.provider),async ie=>{d.thinkingEffort=ie.trim(),await this.plugin.saveSettings()}),g.reasoningSplit&&$(se,"Split",!!d.reasoningSplit,async ie=>{d.reasoningSplit=ie,await this.plugin.saveSettings()})}}})};x(),c(),o(),new E.Setting(n).setName("\u5237\u65B0\u540E\u7AEF Profile").setDesc("\u91CD\u65B0\u4ECE\u540E\u7AEF\u8BFB\u53D6\u5F53\u524D LLM Profile \u5217\u8868\u3002").addButton(d=>{d.setButtonText("\u5237\u65B0"),d.onClick(()=>{o()})}),new E.Setting(n).setName("\u6D4B\u8BD5\u5F53\u524D Profile").setDesc("\u6821\u9A8C\u540E\u7AEF\u5F53\u524D\u5DF2\u751F\u6548\u7684 provider\u3001model\u3001key\uFF0C\u5E76\u5728 DeepSeek / MiniMax \u4E0A\u505A\u4E00\u6B21\u4F4E token \u771F\u5B9E\u63A2\u6D4B\u3002").addButton(d=>{d.setButtonText("\u6D4B\u8BD5"),d.onClick(()=>{f()})}),new E.Setting(n).setName("\u6DFB\u52A0\u914D\u7F6E").setDesc("\u65B0\u589E\u4E00\u4E2A LLM \u914D\u7F6E\u9884\u8BBE\u3002").addButton(d=>{d.setButtonText(r.ok?"\u6DFB\u52A0":"\u8BF7\u5148\u521D\u59CB\u5316\u540E\u7AEF"),d.setDisabled(!r.ok),d.onClick(async()=>{let P=this.plugin.settings.llmProfiles.length===0,C={id:gi(),name:"\u65B0\u914D\u7F6E",provider:"anthropic",model:"claude-sonnet-4-20250514",baseUrl:"",apiKey:"",supportsVision:!1,thinkingMode:"",thinkingEffort:"",thinkingBudgetTokens:"1024",reasoningSplit:!1,isDraft:!0};this.plugin.settings.llmProfiles.push(C),P&&(this.plugin.settings.activeProfileId=C.id),await this.plugin.saveSettings(),x(),c(),i.setText("\u5DF2\u6DFB\u52A0\u65B0\u914D\u7F6E\u8349\u7A3F\u3002\u586B\u5199\u5B8C\u6210\u540E\u70B9\u51FB\u201C\u4FDD\u5B58\u201D\u5199\u5165\u540E\u7AEF .env\u3002")})})}};var pe=require("obsidian"),dn=/\[Image\s+#(\d+)\]/g,Si=/(^|[^0-9A-Za-z_./\\:-])\/([^\s/]*)$/,_i=/(^|[^0-9A-Za-z_./\\:-])@"([^"]*)$/,Ei=/(^|[^0-9A-Za-z_./\\:-])@([^\s"]*)$/,Ti=/(^|[^0-9A-Za-z_./\\:-])@"([^"]+)"(#L\d+(?:-\d+)?)?/g,Ci=/(^|[^0-9A-Za-z_./\\:-])@([^\s"]+)/g,br=4,Mi=10*1024*1024;function kr(t){let{app:e,client:n,elements:r,state:s}=t,i=[],a=1,l={},o=[],c=0,u=null,p=null,f="",x=!1,d=!1,P=0,C=null,y=[];n.listSkills().then(v=>{i=v,te()}).catch(()=>{i=[]}),n.getCapabilities().then(v=>{C=v}).catch(()=>{C=null});let _=()=>{x?x=!1:qn(),qe(),J(),te()},A=()=>{if(d){d=!1;return}te()},M=v=>{if(o.length>0){if(v.key==="ArrowDown"){d=!0,v.preventDefault(),v.stopPropagation(),c=(c+1)%o.length,ee();return}if(v.key==="ArrowUp"){d=!0,v.preventDefault(),v.stopPropagation(),c=(c-1+o.length)%o.length,ee();return}if(v.key==="Tab"||v.key==="Enter"){v.preventDefault(),v.stopPropagation(),O(o[c]);return}if(v.key==="Escape"){d=!0,v.preventDefault(),v.stopPropagation(),o=[],c=0,u=null,ee();return}}},z=v=>{let T=Ni(v);T.length!==0&&(v.preventDefault(),w(T))},U=v=>{Oi(v.dataTransfer?.files)&&(v.preventDefault(),r.inputAreaEl.classList.add("drag-over"))},j=()=>{r.inputAreaEl.classList.remove("drag-over")},X=v=>{r.inputAreaEl.classList.remove("drag-over");let T=un(v.dataTransfer?.files);T.length!==0&&(v.preventDefault(),w(T))},K=()=>{r.hiddenFileInput.click()},S=()=>{let v=un(r.hiddenFileInput.files);r.hiddenFileInput.value="",v.length!==0&&w(v)},D=()=>{h()};r.inputEl.addEventListener("input",_),r.inputEl.addEventListener("keydown",M),r.inputEl.addEventListener("click",A),r.inputEl.addEventListener("keyup",A),r.inputEl.addEventListener("paste",z),r.inputAreaEl.addEventListener("dragover",U),r.inputAreaEl.addEventListener("dragleave",j),r.inputAreaEl.addEventListener("drop",X),r.attachmentBtn.addEventListener("click",K),r.hiddenFileInput.addEventListener("change",S),window.addEventListener("focus",D),y.push(()=>{r.inputEl.removeEventListener("input",_),r.inputEl.removeEventListener("keydown",M),r.inputEl.removeEventListener("click",A),r.inputEl.removeEventListener("keyup",A),r.inputEl.removeEventListener("paste",z),r.inputAreaEl.removeEventListener("dragover",U),r.inputAreaEl.removeEventListener("dragleave",j),r.inputAreaEl.removeEventListener("drop",X),r.attachmentBtn.removeEventListener("click",K),r.hiddenFileInput.removeEventListener("change",S),window.removeEventListener("focus",D)});function m(){let v=r.inputEl.value,T=V(v),L=Di(v),R=$(v,T);return!L.trim()&&R.length===0?null:T.length>0&&C?.supports_vision===!1?(new pe.Notice("\u5F53\u524D\u540E\u7AEF\u6A21\u578B\u672A\u5F00\u542F\u89C6\u89C9\u80FD\u529B\uFF0C\u56FE\u7247\u5DF2\u4FDD\u7559\u5728\u8F93\u5165\u6846\u91CC\uFF0C\u6682\u65F6\u4E0D\u80FD\u53D1\u9001\u3002"),null):{request:{content:v,pasted_contents:T.map(({preview_url:F,size_bytes:q,...Y})=>Y)},displayText:L,displayAttachments:R}}function g(){k(),r.inputEl.value="",qe(),te()}function b(){k(),y.splice(0).forEach(v=>v())}function k(){l={},o=[],c=0,u=null,qn(),r.composerPillsEl.empty(),ee()}async function h(){if(!(typeof navigator>"u"||!navigator.clipboard||typeof navigator.clipboard.read!="function")&&!(Date.now()-P<15e3))try{(await navigator.clipboard.read()).some(L=>L.types.some(R=>R.startsWith("image/")))&&(P=Date.now(),new pe.Notice("\u526A\u8D34\u677F\u91CC\u6709\u56FE\u7247\uFF0C\u53EF\u4EE5\u76F4\u63A5\u7C98\u8D34\u5230\u5BF9\u8BDD\u6846\u3002"))}catch{}}async function w(v){if(Object.keys(l).length+v.length>br){new pe.Notice(`\u6BCF\u6B21\u6700\u591A\u9644\u5E26 ${br} \u5F20\u56FE\u7247\u3002`);return}for(let L of v){if(L.size>Mi){new pe.Notice(`${L.name} \u8D85\u8FC7 10 MB\uFF0C\u5DF2\u8DF3\u8FC7\u3002`);continue}let R=await Fi(L),[F,q]=R.split(",",2);if(!q)continue;let Y=Ui(F)||L.type||"image/png",ge=await Hi(R),mt=a++;l[mt]={id:mt,type:"image",data:q,media_type:Y,filename:L.name||`Image ${mt}`,width:ge?.width,height:ge?.height,preview_url:R,size_bytes:L.size},Me(mt)}Z(),te()}function $(v,T){let L=I(v),R=T.map(F=>({type:"image",filename:F.filename,media_type:F.media_type,width:F.width,height:F.height,preview_url:F.preview_url}));return[...L,...R]}function I(v){let T=Li(v),L=[];for(let R of T){let F=R.path,q=e.vault.getAbstractFileByPath(F);if(q instanceof pe.TFolder){let Y={type:"vault_directory",path:F,entry_count:q.children.length};L.push(Y)}else if(q instanceof pe.TFile){let Y={type:"vault_file",path:F,line_start:R.line_start,line_end:R.line_end};L.push(Y)}}return L}function V(v){let T=Array.from(v.matchAll(dn)).map(F=>Number(F[1])).filter(F=>Number.isFinite(F)),L=[],R=new Set;for(let F of T)R.has(F)||!l[F]||(R.add(F),L.push(l[F]));return L}function J(){let v=new Set(Array.from(r.inputEl.value.matchAll(dn)).map(T=>Number(T[1])));for(let[T,L]of Object.entries(l))v.has(Number(T))||delete l[Number(T)];Z()}function Z(){r.composerPillsEl.empty();for(let v of Object.values(l)){let T=r.composerPillsEl.createDiv({cls:"chat-image-pill"});T.createEl("img",{cls:"chat-image-pill-thumb",attr:{src:v.preview_url,alt:v.filename}}),T.createDiv({cls:"chat-image-pill-label"}).setText(v.filename);let R=T.createEl("button",{cls:"chat-image-pill-remove",attr:{"aria-label":`Remove ${v.filename}`}});R.setText("\xD7"),R.addEventListener("click",()=>{delete l[v.id],r.inputEl.value=r.inputEl.value.replace(new RegExp(`\\s*\\[Image\\s+#${v.id}\\]\\s*`,"g")," ").replace(/[ \t]{2,}/g," ").trim(),qe(),Z(),te()})}r.composerPillsEl.classList.toggle("has-items",Object.keys(l).length>0)}function te(){let v=je();if(v){se(ie(v.query,v.from,v.to),`slash:${v.from}:${v.to}:${v.query}`);return}let T=Ke();if(T){se($e(T.query,T.from,T.to),`mention:${T.from}:${T.to}:${T.query}`);return}se([])}function ee(){if(r.suggestionListEl.empty(),o.length===0){r.suggestionListEl.classList.remove("is-open");return}r.suggestionListEl.classList.add("is-open"),o.forEach((v,T)=>{let L=r.suggestionListEl.createDiv({cls:"chat-suggestion-item"});T===c&&(L.classList.add("is-selected"),window.setTimeout(()=>{L.scrollIntoView({block:"nearest"})},0)),L.createDiv({cls:"chat-suggestion-title"}).setText(v.label),L.createDiv({cls:"chat-suggestion-desc"}).setText(v.description),L.addEventListener("mousedown",q=>{q.preventDefault(),O(v)})})}function O(v){let T=r.inputEl.value,L=T.slice(0,v.replaceFrom),R=T.slice(v.replaceTo);r.inputEl.value=`${L}${v.insertText}${R}`;let F=v.replaceFrom+v.insertText.length;r.inputEl.setSelectionRange(F,F),r.inputEl.focus(),qe(),o=[],u=null,ee(),J()}function N(v){if(o.length>0)return!1;let T=r.inputEl.selectionStart??r.inputEl.value.length,L=r.inputEl.selectionEnd??T;if(T!==L||v==="up"&&!qs(T)||v==="down"&&!Ys(L))return!1;let R=Ks();return R.length===0?!1:p==null?v==="down"?!1:(f=r.inputEl.value,p=R.length-1,ut(R[p]),!0):v==="up"?(p===0||(p-=1,ut(R[p])),!0):p>=R.length-1?(p=null,ut(f),!0):(p+=1,ut(R[p]),!0)}function se(v,T=null){let L=o[c],R=T!=null&&T===u;if(o=v,u=T,o.length===0){c=0,ee();return}if(R&&L){let F=o.findIndex(q=>$i(q,L));if(F>=0){c=F,ee();return}}c=R?Math.min(c,o.length-1):0,ee()}function ie(v,T,L){let R=v.trim().toLowerCase();return i.map(q=>({skill:q,score:Ri(q,R)})).filter(q=>q.score>0||R.length===0).sort((q,Y)=>Y.score-q.score||q.skill.name.localeCompare(Y.skill.name)).slice(0,8).map(({skill:q})=>({kind:"slash",label:`/${q.name}`,description:q.description,replaceFrom:T,replaceTo:L,insertText:`/${q.name} `}))}function $e(v,T,L){let R=v.trim().toLowerCase();return e.vault.getAllLoadedFiles().filter(Ai).map(Y=>({candidate:Y,score:Ii(Y,R)})).filter(Y=>Y.score>0||R.length===0).sort((Y,ge)=>ge.score-Y.score||Y.candidate.path.localeCompare(ge.candidate.path)).slice(0,8).map(({candidate:Y})=>({kind:"mention",label:Y instanceof pe.TFolder?`@${Y.path}/`:`@${Y.path}`,description:Y instanceof pe.TFolder?`${Y.children.length} items`:Y.basename,replaceFrom:T,replaceTo:L,insertText:`${Bi(Y.path)} `}))}function je(){let v=r.inputEl.selectionStart??r.inputEl.value.length,L=r.inputEl.value.slice(0,v).match(Si);if(!L||L.index==null)return null;let R=L.index+L[1].length,F=v;for(;F<r.inputEl.value.length&&!/\s/.test(r.inputEl.value[F]);)F+=1;return{query:L[2]??"",from:R,to:F}}function Ke(){let v=r.inputEl.selectionStart??r.inputEl.value.length,T=r.inputEl.value.slice(0,v),L=T.match(_i);if(L&&L.index!=null){let Y=L.index+L[1].length,ge=v;for(;ge<r.inputEl.value.length&&r.inputEl.value[ge]!=='"';)ge+=1;return r.inputEl.value[ge]==='"'&&(ge+=1),{query:L[2]??"",from:Y,to:ge}}let R=T.match(Ei);if(!R||R.index==null)return null;let F=R.index+R[1].length,q=v;for(;q<r.inputEl.value.length&&!/\s/.test(r.inputEl.value[q]);)q+=1;return{query:R[2]??"",from:F,to:q}}function Me(v){let T=`[Image #${v}]`;De(`${Ws()?" ":""}${T} `),qe()}function De(v){let T=r.inputEl.selectionStart??r.inputEl.value.length,L=r.inputEl.selectionEnd??T,R=r.inputEl.value;r.inputEl.value=`${R.slice(0,T)}${v}${R.slice(L)}`;let F=T+v.length;r.inputEl.setSelectionRange(F,F),r.inputEl.focus()}function ut(v){x=!0,r.inputEl.value=v;let T=v.length;r.inputEl.setSelectionRange(T,T),r.inputEl.focus(),qe(),J(),te()}function qn(){p=null,f=""}function Ks(){return s.messages.filter(v=>v.role==="user"&&!!v.content.trim()).map(v=>v.content)}function qs(v){return!r.inputEl.value.slice(0,v).includes(`
`)}function Ys(v){return!r.inputEl.value.slice(v).includes(`
`)}function Ws(){let v=r.inputEl.selectionStart??r.inputEl.value.length,T=r.inputEl.value[v-1];return!!(T&&!/\s/.test(T))}function qe(){r.inputEl.style.height="auto",r.inputEl.style.height=`${Math.min(r.inputEl.scrollHeight,120)}px`}return{getSubmitPayload:m,navigateHistory:N,clear:g,destroy:b}}function Di(t){return t.replace(dn,"").replace(/[ \t]{2,}/g," ").replace(/\n{3,}/g,`

`).trim()}function Li(t){let e=[],n=new Set;for(let r of t.matchAll(Ti)){let s=`${r[2]??""}${r[3]??""}`;yr(e,n,s)}for(let r of t.matchAll(Ci)){let s=(r[2]??"").replace(/[.,;:!?]+$/,"");s.startsWith('"')||yr(e,n,s)}return e}function yr(t,e,n){if(!n||e.has(n))return;e.add(n);let r=n.match(/^(.*)#L(\d+)(?:-(\d+))?$/);if(!r){t.push({path:n});return}let s=Number(r[2]),i=Number(r[3]??r[2]);t.push({path:r[1],line_start:Math.min(s,i),line_end:Math.max(s,i)})}function Ri(t,e){if(!e)return 1;let n=t.name.toLowerCase(),r=t.description.toLowerCase();return n.startsWith(e)?5:n.includes(e)?4:(t.aliases??[]).some(s=>s.toLowerCase().startsWith(e))?3.5:r.includes(e)?2:0}function Ai(t){return t instanceof pe.TFile||t instanceof pe.TFolder?!!t.path:!1}function Ii(t,e){if(!e)return 1;let n=t.path.toLowerCase(),r=t.name.toLowerCase();return r.startsWith(e)?5:n.startsWith(e)?4.5:r.includes(e)?4:n.includes(e)?3:0}function Bi(t){return/\s/.test(t)?`@"${t}"`:`@${t}`}function $i(t,e){return t.kind===e.kind&&t.label===e.label&&t.insertText===e.insertText&&t.replaceFrom===e.replaceFrom&&t.replaceTo===e.replaceTo}function Ni(t){return Array.from(t.clipboardData?.items??[]).filter(n=>n.type.startsWith("image/")).map(n=>n.getAsFile()).filter(n=>n!=null)}function un(t){return Array.from(t??[]).filter(e=>e.type.startsWith("image/"))}function Oi(t){return un(t).length>0}function Fi(t){return new Promise((e,n)=>{let r=new FileReader;r.onload=()=>e(String(r.result)),r.onerror=()=>n(r.error),r.readAsDataURL(t)})}function Ui(t){let e=t.match(/^data:([^;]+);base64$/);return e?e[1]:null}function Hi(t){return new Promise(e=>{let n=new Image;n.onload=()=>e({width:n.width,height:n.height}),n.onerror=()=>e(null),n.src=t})}var xr=require("node:fs"),Ze=require("node:path"),Ae=require("obsidian");function Pr(t){let{app:e,client:n,plugin:r,rootEl:s,openPluginSettings:i}=t,a=null;function l(){a=null,s.empty(),s.classList.remove("is-open","is-writing","is-missing-template")}function o(){let u=a;if(s.empty(),s.classList.remove("is-open","is-writing","is-missing-template"),!u)return;let p=zi(e,r);s.classList.add("is-open"),u.writing&&s.classList.add("is-writing"),p||s.classList.add("is-missing-template");let f=s.createDiv({cls:"chat-diary-prompt-panel"}),x=f.createDiv({cls:"chat-diary-prompt-text"});x.createDiv({cls:"chat-diary-prompt-title",text:"Loop \u4EFB\u52A1\u5DF2\u5B8C\u6210"}),x.createDiv({cls:"chat-diary-prompt-body",text:p?"\u8981\u628A\u8FD9\u6B21\u5FAA\u73AF\u4EFB\u52A1\u7684\u603B\u7ED3\u5199\u5165\u4ECA\u65E5\u65E5\u8BB0\u5417\uFF1F":"\u5148\u914D\u7F6E\u65E5\u8BB0\u6A21\u677F\u540E\u624D\u80FD\u5199\u5165\u4ECA\u65E5\u65E5\u8BB0\u3002"}),x.createDiv({cls:"chat-diary-prompt-preview",text:qi(u.summary)});let d=f.createDiv({cls:"chat-diary-prompt-actions"});if(p){let y=d.createEl("button",{cls:"chat-diary-prompt-btn is-primary",text:u.writing?"\u5199\u5165\u4E2D...":"\u5199\u5165\u4ECA\u65E5\u65E5\u8BB0"});y.disabled=u.writing,y.addEventListener("click",()=>{c()});let _=d.createEl("button",{cls:"chat-diary-prompt-btn",text:"\u8DF3\u8FC7"});_.disabled=u.writing,_.addEventListener("click",l);return}d.createEl("button",{cls:"chat-diary-prompt-btn is-primary",text:"\u53BB\u8BBE\u7F6E"}).addEventListener("click",()=>{i()||new Ae.Notice("\u65E0\u6CD5\u81EA\u52A8\u6253\u5F00 Crabby \u8BBE\u7F6E\uFF0C\u8BF7\u4ECE Obsidian \u8BBE\u7F6E\u91CC\u6253\u5F00\u63D2\u4EF6\u8BBE\u7F6E\u3002")}),d.createEl("button",{cls:"chat-diary-prompt-btn",text:"\u5173\u95ED"}).addEventListener("click",l)}async function c(){let u=a;if(!(!u||u.writing)){u.writing=!0,o();try{let p=await r.ensureBackendVaultPathSynced(n);if(!p.ok)throw new Error(p.message);let f=await n.writeDiaryEntry({session_id:u.sessionId,conversation_id:u.conversationId,period:"daily",date:Yi(new Date),summary:u.summary,topics:["loop"],domains:["task"],memory_links:[],entry_key:u.entryKey});if(f.is_error||f.status==="error")throw new Error(f.output||"\u65E5\u8BB0\u5199\u5165\u5931\u8D25\u3002");a===u&&l();let x=!!f.metadata?.deduplicated;new Ae.Notice(x?"\u4ECA\u65E5\u65E5\u8BB0\u91CC\u5DF2\u6709\u8FD9\u6761 Loop \u603B\u7ED3\u3002":"\u5DF2\u5199\u5165\u4ECA\u65E5\u65E5\u8BB0\u3002")}catch(p){a===u&&(u.writing=!1,o());let f=p instanceof Error?p.message:String(p);new Ae.Notice(`\u5199\u5165\u4ECA\u65E5\u65E5\u8BB0\u5931\u8D25\uFF1A${f}`)}}}return{showLoopStopResult(u,p,f){if(u.is_error||u.status==="error")return;let x=String(u.output??"").trim();if(!x||!p||!f)return;let d=ji(u);if(!d)return;let P=Vi(d);a={sessionId:p,conversationId:f,summary:x,entryKey:`loop:${P}:completion`,writing:!1},o()},hide:l,destroy:l}}function zi(t,e){let n=e.settings.diary?.templatePaths?.daily?.trim();if(!n)return!1;let r=(0,Ae.normalizePath)(n);if(t.vault.getAbstractFileByPath(r)instanceof Ae.TFile)return!0;let i=e.getCurrentVaultPath().trim();if(!i)return!1;let a=(0,Ze.resolve)(i),l=(0,Ze.resolve)(a,r);if(!Ki(l,a))return!1;try{return(0,xr.statSync)(l).isFile()}catch{return!1}}function Vi(t){return(t.replace(/\r|\n/g," ").replace(/-->/g,"--").trim()||"unknown").slice(0,150)}function ji(t){let e=t.metadata?.job_id;return typeof e!="string"?null:e.trim()||null}function Ki(t,e){if(t===e)return!0;let n=e.endsWith(Ze.sep)?e:`${e}${Ze.sep}`;return t.startsWith(n)}function qi(t,e=260){let n=t.replace(/\s+/g," ").trim();return n.length<=e?n:`${n.slice(0,e).trim()}...`}function Yi(t){let e=t.getFullYear(),n=String(t.getMonth()+1).padStart(2,"0"),r=String(t.getDate()).padStart(2,"0");return`${e}-${n}-${r}`}var Tt=`
    ...
    `;let i=r.createDiv({cls:"custom-select-dropdown"}),a=[],l=[],o=()=>{l=[{kind:"auto",id:"auto",label:"Auto"},{kind:"none",id:"none",label:"No Persona"},...a.map(y=>({kind:"manual",id:y.id,label:y.title}))]},c=y=>y?a.find(_=>_.id===y)?.title??y:null,u=y=>y.mode==="none"?"none":y.mode==="manual"?y.manual_persona_id??"manual":"auto",p=y=>{if(y.mode==="none")return"No Persona";if(y.mode==="manual")return c(y.manual_persona_id)??"Manual";let _=c(y.active_persona_id);return _?`Auto / ${_}`:"Auto"},f=()=>{s.querySelector("span")?.setText(p(n.personaState));let y=u(n.personaState);Array.from(i.children).forEach(_=>{let A=_;A.classList.toggle("selected",A.dataset.optionKey===y)})},x=y=>{n.personaState={...Le(),...y},f()},d=y=>y.kind==="none"?{mode:"none",manual_persona_id:null,active_persona_id:null,source:"none",status:"disabled"}:y.kind==="manual"?{mode:"manual",manual_persona_id:y.id,active_persona_id:y.id,source:"manual",status:"manual"}:Le(),P=()=>{i.empty(),o();for(let y of l){let _=i.createDiv({cls:"custom-select-option"});_.dataset.optionKey=y.kind==="manual"?y.id:y.kind,_.createEl("span",{cls:"cso-name"}).setText(y.label),_.createEl("span",{cls:"cso-provider cso-meta"}).setText(y.kind==="auto"?"AUTO":y.kind==="none"?"OFF":"MANUAL"),_.addEventListener("click",async z=>{z.stopPropagation(),r.classList.remove("open");let U=n.personaState,j=d(y);x(j);let X=e.sessionId;if(X)try{let K=await e.patchSession(X,{persona_mode:j.mode,manual_persona_id:j.manual_persona_id});x(K.persona_state)}catch(K){x(U);let S=K instanceof Error?K.message:String(K);new Lr.Notice(`Persona switch failed: ${S}`)}})}f()};e.listPersonas().then(y=>{a=y,P()}).catch(y=>{console.warn("[ChatView] listPersonas failed:",y),P()}),P(),s.addEventListener("click",y=>{y.stopPropagation(),y.preventDefault(),r.classList.toggle("open")});let C=y=>{r.contains(y.target)||r.classList.remove("open")};return document.addEventListener("click",C),{setPersonaState:x,destroy:()=>{document.removeEventListener("click",C)}}}var Ct=require("obsidian");function mn(t){return t.name.trim()||t.model.trim()||ue(t.provider).label}function Wi(t){return ue(t.provider).label.toUpperCase()}function Ar(t,e,n){let r=t.createDiv({cls:"chat-custom-select"}),s=r.createDiv({cls:"custom-select-trigger"});s.innerHTML=`<span>Select Model</span>
      <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
    `;let i=r.createDiv({cls:"custom-select-dropdown"}),a=[],l=()=>e.settings.llmProfiles.filter(x=>!ve(x)),o=()=>l().find(x=>x.id===e.settings.activeProfileId)??l()[0],c=()=>{let x=o();s.querySelector("span")?.setText(x?mn(x):"Select Model"),a.forEach(({optionEl:d,profileId:P})=>{d.classList.toggle("selected",P===e.settings.activeProfileId)})},u=()=>{i.empty(),a=[];let x=l();if(x.length===0){i.createDiv({cls:"custom-select-option custom-select-option-empty"}).setText("No LLM profiles"),c();return}x.forEach(d=>{let P=i.createDiv({cls:"custom-select-option"});a.push({profileId:d.id,optionEl:P});let C=P.createDiv({cls:"cso-label"});C.createEl("span",{cls:"cso-name"}).setText(mn(d)),C.createEl("span",{cls:"cso-model"}).setText(`${ue(d.provider).label} / ${d.model}`);let A=P.createEl("span",{cls:"cso-provider"});A.setText(Wi(d)),A.setAttribute("data-provider",d.provider),P.addEventListener("click",async M=>{M.stopPropagation(),r.classList.remove("open");let z=l().find(U=>U.id===d.id)??d;if(z.id===e.settings.activeProfileId){c();return}try{let U=await We(e.settings,z.id,n);if(U.ok){await e.saveSettings(),u(),new Ct.Notice(`Switched to model: ${mn(z)}`);return}c(),new Ct.Notice(`Profile switch failed: ${U.message}`)}catch(U){c();let j=U instanceof Error?U.message:String(U);new Ct.Notice(`Profile switch failed: ${j}`)}})}),c()};u(),s.addEventListener("click",x=>{x.stopPropagation(),x.preventDefault(),u(),r.classList.toggle("open")});let p=x=>{r.contains(x.target)||r.classList.remove("open")},f=()=>{u()};return document.addEventListener("click",p),document.addEventListener(Oe,f),()=>{document.removeEventListener("click",p),document.removeEventListener(Oe,f)}}var xe=require("obsidian");var Ir=require("obsidian"),Gi="<think>",Ji="</think>",Xi="<thinking>",Zi="</thinking>",Br="<think-json>",$r="</think-json>",Qi="Crabby",Nr=[{open:Br,close:$r,encoded:!0},{open:Gi,close:Ji,allowNested:!0},{open:Xi,close:Zi,allowNested:!0}];function pn(t){let e=t.createDiv({cls:"chat-assistant-header"});return e.createSpan({cls:"chat-assistant-name",text:Qi}),e}function Or(t,e,n,r){n.empty();let s=gn(r);if(s.thoughtText&&Ur(n,s.thoughtText),s.visibleMarkdown.trim()){let i=n.createDiv({cls:"chat-assistant-markdown"});Ir.MarkdownRenderer.render(t,s.visibleMarkdown,i,"",e)}}function Fr(t){t.empty();let e=t.createDiv({cls:"chat-assistant-shell"});pn(e);let n=e.createDiv({cls:"chat-assistant-content"}),r=null,s=null;return{render(i,a){let l=a.trim();l&&(r?r.updateThoughtText(l):r=Ur(n,l,{streaming:!0})),i?(s||(s=n.createDiv({cls:"chat-assistant-markdown chat-assistant-streaming-text"})),s.setText(i)):s&&(s.remove(),s=null)}}}function Mt(t,e){let n=t.trim();return n?`${Br}${ia(n)}${$r}
...
${e.close}`;return t.endsWith(i)?t.length-e.close.length:-1}function sa(t,e){return((e.encoded?aa(t):t)??t).trim()}function ia(t){return JSON.stringify(t).replace(/[<>&]/g,e=>e==="<"?"\\u003c":e===">"?"\\u003e":"\\u0026")}function aa(t){try{let e=JSON.parse(t);return typeof e=="string"?e:null}catch{return null}}function Ur(t,e,n={}){let r=t.createDiv({cls:n.streaming?"chat-thought-block streaming":"chat-thought-block"}),s=r.createDiv({cls:"chat-thought-header"});s.setAttribute("role","button"),s.setAttribute("tabindex","0"),s.setAttribute("aria-expanded","false"),s.createSpan({cls:"chat-thought-title"}).setText("\u601D\u7EF4\u94FE");let a=s.createSpan({cls:"chat-thought-preview"}),l=s.createSpan({cls:"chat-thought-chevron"});l.setText(">");let o=r.createDiv({cls:"chat-thought-body"}),c=p=>{let f=la(p);a.classList.toggle("is-empty",!f),a.setText(f?f.slice(0,72)+(f.length>72?"...":""):""),o.setText(p)},u=()=>{let p=!r.classList.contains("expanded");r.classList.toggle("expanded",p),s.setAttribute("aria-expanded",p?"true":"false"),l.setText(p?"v":">")};return s.addEventListener("click",u),s.addEventListener("keydown",p=>{(p.key==="Enter"||p.key===" ")&&(p.preventDefault(),u())}),c(e),{updateThoughtText:c}}function oa(t){return t.replace(/\n{3,}/g,`

`).trim()}function la(t){return t.trim().split(`
`).find(e=>e.trim())}function ca(t){if(t==null||Number.isNaN(t))return"\u672A\u77E5\u65F6\u95F4";let e=t>1e10?t:t*1e3;if(e===0)return"\u65E9\u671F\u4F1A\u8BDD";let n=Date.now()-e;if(n<0)return"\u521A\u521A";let r=Math.floor(n/6e4);if(r<1)return"\u521A\u521A";if(r<60)return`${r} \u5206\u949F\u524D`;let s=Math.floor(r/60);if(s<24)return`${s} \u5C0F\u65F6\u524D`;let i=Math.floor(s/24);if(i<7)return`${i} \u5929\u524D`;let a=new Date(e);return`${a.getFullYear()}/${a.getMonth()+1}/${a.getDate()}`}function da(t){let e=t.reasoning_details;return Array.isArray(e)?e.map(n=>typeof n=="object"&&n!==null&&typeof n.text=="string"?n.text:"").join(""):typeof t.thinking=="string"?t.thinking:""}var hn=class extends xe.Modal{constructor(n,r,s,i){super(n);this.sourcePreview=r;this.suggestedTitle=s;this.resolved=!1;this.resolve=i}onOpen(){let{contentEl:n}=this;n.empty(),n.addClass("fork-conversation-modal"),n.createEl("h2",{text:"\u786E\u8BA4\u5206\u53C9\u6807\u9898"});let r=n.createDiv({cls:"fork-conversation-preview"});r.createEl("div",{cls:"fork-conversation-label",text:"\u6765\u6E90\u6D88\u606F"}),r.createEl("div",{cls:"fork-conversation-text",text:this.sourcePreview});let s=n.createDiv({cls:"fork-conversation-title"});s.createEl("div",{cls:"fork-conversation-label",text:"\u5206\u652F\u6807\u9898"}),this.titleInput=s.createEl("input",{cls:"fork-conversation-input",attr:{type:"text",value:this.suggestedTitle,spellcheck:"false"}}),this.titleInput.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),this.submit()),o.key==="Escape"&&(o.preventDefault(),this.close())});let i=n.createDiv({cls:"fork-conversation-actions"});i.createEl("button",{cls:"mod-muted",text:"\u53D6\u6D88"}).addEventListener("click",()=>this.close()),i.createEl("button",{cls:"mod-cta",text:"\u5206\u53C9"}).addEventListener("click",()=>this.submit()),window.requestAnimationFrame(()=>{this.titleInput.focus(),this.titleInput.select()})}onClose(){this.resolved||(this.resolved=!0,this.resolve(null)),this.contentEl.removeClass("fork-conversation-modal"),this.contentEl.empty()}submit(){this.resolved||(this.resolved=!0,this.resolve(this.titleInput.value.trim()),this.close())}};function ua(t,e,n){return new Promise(r=>{new hn(t,e,n,r).open()})}function Hr(t){return(gn(t).visibleMarkdown||t).replace(/\s+/g," ").trim()}function ma(t){return Hr(t).slice(0,40)||"\u65B0\u5206\u652F"}function pa(t){return Hr(t).slice(0,160)||"\uFF08\u7A7A\u6D88\u606F\uFF09"}function ga(t){let e=new Map;for(let s of t)e.set(s.id,{...s,children:[]});let n=[];for(let s of e.values()){let i=s.parent_id??"",a=i?e.get(i):void 0;a?a.children.push(s):n.push(s)}let r=s=>{s.sort((i,a)=>i.created_at!==a.created_at?i.created_at-a.created_at:i.id.localeCompare(a.id));for(let i of s)i.children.length>0&&r(i.children)};return r(n),n}function zr(t){let{app:e,client:n,composer:r,elements:s,state:i,transcript:a,persona:l}=t;a.setForkHandler(S=>{U(S)});async function o(){s.sessionListEl.empty(),s.sessionListEl.createDiv({cls:"session-loading"}).setText("\u52A0\u8F7D\u4E2D...");try{let D=await n.listSessions();if(s.sessionListEl.empty(),D.length===0){s.sessionListEl.createDiv({cls:"session-empty"}).setText("\u6682\u65E0\u5386\u53F2\u4F1A\u8BDD");return}for(let m of D)j(m)}catch{s.sessionListEl.empty(),s.sessionListEl.createDiv({cls:"session-error"}).setText("\u52A0\u8F7D\u5931\u8D25\uFF0C\u8BF7\u68C0\u67E5\u540E\u7AEF\u8FDE\u63A5")}}async function c(){if(!i.treePanelOpen)return;s.treeListEl.empty(),s.treeListEl.createDiv({cls:"conversation-tree-loading"}).setText("\u52A0\u8F7D\u4E2D...");let D=n.sessionId;if(!D){s.treeListEl.empty(),s.treeListEl.createDiv({cls:"conversation-tree-empty"}).setText("\u5F53\u524D\u8FD8\u6CA1\u6709\u53EF\u663E\u793A\u7684\u4F1A\u8BDD\u6811"),s.treePanelTitleEl.setText("\u4F1A\u8BDD\u6811");return}try{let[m,g]=await Promise.all([n.getSession(D),n.listConversations(D)]);if(!i.treePanelOpen||n.sessionId!==D)return;if(s.treePanelTitleEl.setText(m.title?`\u4F1A\u8BDD\u6811 \xB7 ${m.title}`:"\u4F1A\u8BDD\u6811"),s.treeListEl.empty(),g.length===0){s.treeListEl.createDiv({cls:"conversation-tree-empty"}).setText("\u5F53\u524D\u4F1A\u8BDD\u5C1A\u65E0\u5206\u652F");return}let b=ga(g);X(b,s.treeListEl,m.id)}catch(m){if(!i.treePanelOpen)return;s.treeListEl.empty();let g=m instanceof Error?m.message:String(m);s.treeListEl.createDiv({cls:"conversation-tree-error"}).setText(`\u4F1A\u8BDD\u6811\u52A0\u8F7D\u5931\u8D25\uFF1A${g}`)}}function u(){i.sessionPanelOpen=!0,i.treePanelOpen=!1,s.sessionPanelEl.addClass("open"),s.treePanelEl.removeClass("open")}function p(){i.sessionPanelOpen=!1,s.sessionPanelEl.removeClass("open")}function f(){i.treePanelOpen=!0,i.sessionPanelOpen=!1,s.treePanelEl.addClass("open"),s.sessionPanelEl.removeClass("open")}function x(){i.treePanelOpen=!1,s.treePanelEl.removeClass("open")}function d(){if(i.sessionPanelOpen){p();return}u(),o()}function P(){if(i.treePanelOpen){x();return}f(),c()}function C(){p(),x(),n.disconnect(),a.clearConversationUi(),r.clear(),l.setPersonaState(Le()),s.sessionTitleEl.setText("\u65B0\u4F1A\u8BDD"),s.treePanelTitleEl.setText("\u4F1A\u8BDD\u6811"),s.treeListEl.empty(),a.appendMessage("assistant","\u4F60\u597D\uFF01\u65B0\u4F1A\u8BDD\u5DF2\u7ECF\u5F00\u59CB\u4E86\uFF0C\u6709\u4EC0\u4E48\u53EF\u4EE5\u5E2E\u4F60\u7684\uFF1F")}async function y(S){try{let D=S.active_conversation_id,m=[],g=null;try{m=await n.getConversationMessages(S.id,D)}catch(k){console.warn("[ChatView] getConversationMessages failed:",k)}try{g=await n.getConversationContextStats(S.id,D)}catch(k){console.warn("[ChatView] getConversationContextStats failed:",k)}n.setSession(S.id,D),l.setPersonaState(S.persona_state??Le()),s.sessionTitleEl.setText(S.title||"\u672A\u547D\u540D\u4F1A\u8BDD"),a.clearConversationUi(),r.clear();let b=new Map;for(let k of m)if(k.role==="user"&&Array.isArray(k.content)){for(let h of k.content)if(h.type==="tool_result"&&h.tool_use_id){let w=typeof h.content=="string"?h.content:JSON.stringify(h.content||""),$=h.ui&&typeof h.ui=="object"?h.ui:{};b.set(h.tool_use_id,{id:h.tool_use_id,tool_use_id:h.tool_use_id,output:w,...$})}}for(let k of m)k.role==="user"?_(k):k.role==="assistant"&&A(k,b);g&&a.updateContextBar(g),a.scrollToBottom(!0),i.treePanelOpen&&await c()}catch(D){let m=D instanceof Error?D.message:String(D);console.error("[ChatView] switchToSession failed:",D),new xe.Notice(`\u5207\u6362\u4F1A\u8BDD\u5931\u8D25: ${m}`)}}function _(S){let D=Array.isArray(S.attachments)?S.attachments:[];if(typeof S.text=="string"){a.appendMessage("user",S.text,!1,D,S.message_id);return}let m=!1;if(typeof S.content=="string")a.appendMessage("user",S.content,!1,D,S.message_id),m=!0;else if(Array.isArray(S.content)){let g=S.content.filter(b=>b.type==="text"&&b.text).map(b=>b.text).join(`
`);(g||D.length>0)&&(a.appendMessage("user",g,!1,D,S.message_id),m=!0)}!m&&!Array.isArray(S.content)&&S.content&&a.appendMessage("user",JSON.stringify(S.content),!1,D,S.message_id)}function A(S,D){if(Array.isArray(S.content)){let m="",g="",b=!1,k=()=>{let h=Mt(m,g);h.trim()&&(a.appendMessage("assistant",h,!1,[],!b&&S.message_id?S.message_id:void 0),b=!0),m="",g=""};for(let h of S.content)h.type==="reasoning_details"||h.type==="thinking"?m+=da(h):h.type==="text"&&h.text?g+=`${g?`
`:""}${h.text}`:h.type==="tool_use"&&h.name&&(k(),a.renderHistoricalTool({id:h.id,tool_use_id:h.id,name:h.name,tool:h.name,output:"(no output)",...D.get(h.id)||{}}));k();return}typeof S.content=="string"&&S.content&&a.appendMessage("assistant",S.content,!1,[],S.message_id)}async function M(S){try{await n.deleteSession(S),new xe.Notice("\u4F1A\u8BDD\u5DF2\u5220\u9664"),await o(),n.sessionId===null&&(x(),s.treePanelTitleEl.setText("\u4F1A\u8BDD\u6811"),s.treeListEl.empty())}catch{new xe.Notice("\u5220\u9664\u5931\u8D25")}}async function z(S){if(n.sessionId===S)try{let m=(await n.listSessions()).find(g=>g.id===S);if(!m)return;s.sessionTitleEl.getText()==="\u65B0\u4F1A\u8BDD"&&m.title&&s.sessionTitleEl.setText(m.title),i.treePanelOpen&&(s.treePanelTitleEl.setText(m.title?`\u4F1A\u8BDD\u6811 \xB7 ${m.title}`:"\u4F1A\u8BDD\u6811"),c())}catch{}}async function U(S){if(i.isSending){new xe.Notice("\u5F53\u524D\u6B63\u5728\u56DE\u590D\uFF0C\u8BF7\u5148\u5B8C\u6210\u540E\u518D\u5206\u53C9");return}let D=n.sessionId,m=n.conversationId;if(!D||!m){new xe.Notice("\u5F53\u524D\u6CA1\u6709\u53EF\u5206\u53C9\u7684\u4F1A\u8BDD");return}let g=ma(S.content),b=pa(S.content),k=await ua(e,b,g);if(k!==null)try{let h=await n.forkConversation(D,m,S.messageId,k);await y(h)}catch(h){let w=h instanceof Error?h.message:String(h);new xe.Notice(`\u5206\u53C9\u5931\u8D25: ${w}`)}}function j(S){let D=s.sessionListEl.createDiv({cls:"session-card"}),m=n.sessionId===S.id;m&&D.addClass("active");let g=D.createDiv({cls:"session-card-content"});g.createDiv({cls:"session-card-title"}).setText(S.title||"\u672A\u547D\u540D\u4F1A\u8BDD");let k=g.createDiv({cls:"session-card-meta"}),h=S.turn_count>0?`${S.turn_count} \u6B21\u5BF9\u8BDD`:`${S.message_count} \u6761\u6D88\u606F`;if(k.setText(`${h} \xB7 ${ca(S.created_at)}`),m&&g.createEl("span",{cls:"session-card-badge"}).setText("\u5F53\u524D"),g.addEventListener("click",()=>{p(),y(S)}),!m){let w=D.createEl("button",{cls:"session-card-delete",attr:{"aria-label":"\u5220\u9664\u4F1A\u8BDD"}});w.innerHTML=Mr,w.addEventListener("click",$=>{$.stopPropagation(),M(S.id)})}}function X(S,D,m){for(let g of S){let b=D.createDiv({cls:"conversation-tree-branch"}),k=b.createEl("button",{cls:"conversation-tree-node",attr:{type:"button","aria-pressed":g.active?"true":"false",title:g.active?"\u5F53\u524D\u5206\u652F":"\u5207\u6362\u5230\u8BE5\u5206\u652F"}});g.active&&k.addClass("active");let h=k.createDiv({cls:"conversation-tree-node-main"});if(h.createDiv({cls:"conversation-tree-node-title"}).setText(g.title||"\u672A\u547D\u540D\u5206\u652F"),h.createSpan({cls:"conversation-tree-node-badge"}).setText(g.active?"\u5F53\u524D":`v${g.revision}`),k.createDiv({cls:"conversation-tree-node-meta"}).setText([`${g.message_count} \u6761`,g.fork_message_id?`fork ${g.fork_message_id.slice(0,8)}`:"",g.parent_id?`parent ${g.parent_id.slice(0,8)}`:"root"].filter(Boolean).join(" \xB7 ")),k.addEventListener("click",()=>{if(!g.active){if(i.isSending){new xe.Notice("\u5F53\u524D\u6B63\u5728\u56DE\u590D\uFF0C\u8BF7\u5148\u5B8C\u6210\u540E\u518D\u5207\u6362\u5206\u652F");return}K(m,g.id)}}),g.children.length>0){let V=b.createDiv({cls:"conversation-tree-children"});X(g.children,V,m)}}}async function K(S,D){try{let m=await n.patchSession(S,{active_conversation_id:D});await y(m)}catch(m){let g=m instanceof Error?m.message:String(m);new xe.Notice(`\u5207\u6362\u5206\u652F\u5931\u8D25: ${g}`)}}return{handleNewSession:C,toggleSessionPanel:d,toggleTreePanel:P,loadSessionList:o,loadConversationTree:c,switchToSession:y,deleteSessionConfirm:M,syncCurrentSessionTitle:z}}var Vr="crabby-chat-styles",jr=`
  .crabby-chat {
    position: relative;
    display: flex;
...
`)}function Gr(t){let{app:e,client:n,component:r,elements:s,state:i}=t,a=null;function l(){let m=Array.from(s.minimapEl.querySelectorAll(".chat-minimap-dot")),g=m.length;if(g===0)return;let b=10,k=64,h=24,w=40,$=12,I=s.minimapEl.clientHeight-k-h,V=g===1?0:Math.max($,Math.min(w,(I-b)/(g-1))),J=b+(g-1)*V,Z=k+Math.max(0,(I-J)/2);m.forEach((te,ee)=>{te.style.top=`${Z+ee*V}px`})}function o(m=!1){if(m){requestAnimationFrame(()=>{s.messagesEl.scrollTop=s.messagesEl.scrollHeight});return}let{scrollTop:g,scrollHeight:b,clientHeight:k}=s.messagesEl;b-g-k<150&&(s.messagesEl.scrollTop=b)}function c(m,g,b){m.classList.remove("running"),m.classList.add("done");let k=m.querySelector(".chat-tool-header");if(k){k.empty(),k.createSpan({cls:"chat-tool-icon"}).setText("\u2705"),k.createSpan({cls:"chat-tool-name"}).setText(g);let I=bn(b);I&&k.createSpan({cls:"chat-tool-preview"}).setText(I.slice(0,72)+(I.length>72?"\u2026":""));let V=k.createSpan({cls:"chat-tool-chevron",text:"\u25BE"});k.addEventListener("click",()=>{m.classList.toggle("expanded",!m.classList.contains("expanded")),V.setText(m.classList.contains("expanded")?"\u25B4":"\u25BE")})}let h=m.querySelector(".chat-tool-terminal");h&&(h.empty(),h.setText(b||"(no output)"))}function u(m,g,b=""){let k=fn(g,b),h=qr(k),w=Ca(k),$=Ta(k),I=Yr(k);m.classList.remove("running"),m.classList.add("done"),m.classList.toggle("error",I==="error"),m.classList.toggle("warning",I==="warning"),m.classList.toggle("success",I!=="error"&&I!=="warning");let V=m.querySelector(".chat-tool-header");if(V){V.empty(),V.createSpan({cls:"chat-tool-icon"}).setText(fa(I)),V.createSpan({cls:"chat-tool-name"}).setText(h);let ee=ya(k);V.createSpan({cls:"chat-tool-status"}).setText(ee?`${vn(I)} \xB7 ${ee}`:vn(I));let N=bn($);N&&V.createSpan({cls:"chat-tool-preview"}).setText(N.slice(0,72)+(N.length>72?"...":""));let se=V.createSpan({cls:"chat-tool-chevron",text:">"});V.addEventListener("click",()=>{m.classList.toggle("expanded",!m.classList.contains("expanded")),se.setText(m.classList.contains("expanded")?"v":">")})}let J=m.querySelector(".chat-tool-terminal");J&&(J.empty(),J.setText(w))}function p(m,g,b=!0,k=[],h){i.messages.push({role:m,content:g,attachments:k,messageId:h});let w=s.messagesEl.createDiv({cls:`chat-msg ${m}`});if(h&&(w.dataset.messageId=h),m==="user"){let $=s.minimapEl.createDiv({cls:"chat-minimap-dot"});$.setAttribute("title",g.slice(0,30)),$.addEventListener("click",()=>{w.scrollIntoView({behavior:"smooth",block:"start"})}),i.userMsgRefs.push({dot:$,msgEl:w}),l();let I=w.createDiv({cls:"chat-msg-bubble"});P(I,k),g&&I.createDiv({cls:"chat-msg-text"}).setText(g)}else m==="assistant"&&g?f(w,g,h):g&&w.setText(g);o(b)}function f(m,g,b){m.empty(),b&&(m.dataset.messageId=b);let k=m.createDiv({cls:"chat-assistant-shell"}),h=pn(k);b&&a&&d(h,b,g,"assistant");let w=k.createDiv({cls:"chat-assistant-content"});Or(e,r,w,g)}function x(m){if(!m)return!1;let g=-1;for(let k=i.messages.length-1;k>=0;k-=1)if(i.messages[k].role==="user"){g=k;break}if(g<0)return!1;i.messages[g].messageId=m;let b=i.userMsgRefs[i.userMsgRefs.length-1];return b?(b.msgEl.dataset.messageId=m,!0):!1}function d(m,g,b,k){for(let $ of Array.from(m.children))$.classList.contains("chat-msg-action-row")&&$.remove();let h=m.createDiv({cls:"chat-msg-action-row"}),w=h.createEl("button",{cls:"chat-msg-fork-btn",attr:{type:"button","aria-label":"\u4ECE\u6B64\u6D88\u606F\u5206\u53C9",title:"\u4ECE\u6B64\u6D88\u606F\u5206\u53C9"}});w.innerHTML=Tr,(0,Dt.setTooltip)(w,"\u4ECE\u6B64\u6D88\u606F\u5206\u53C9",{placement:"top",delay:120}),w.addEventListener("click",$=>{$.preventDefault(),$.stopPropagation(),a?.({messageId:g,content:b,role:k})}),!m.classList.contains("chat-assistant-header")&&m.firstElementChild!==h&&m.insertBefore(h,m.firstChild)}function P(m,g){if(g.length===0)return;let b=g.filter(w=>w.type==="image");if(b.length>0){let w=m.createDiv({cls:"chat-msg-images"});for(let $ of b){let I=$.preview_url??($.attachment_id?n.getAttachmentUrl($.attachment_id):"");I&&w.createEl("img",{cls:"chat-msg-image",attr:{src:I,alt:$.filename??"image",loading:"lazy"}})}}let k=g.filter(w=>w.type!=="image");if(k.length===0)return;let h=m.createDiv({cls:"chat-msg-attachment-row"});for(let w of k){let $=h.createDiv({cls:"chat-msg-attachment"}),I=w.type==="vault_directory"?`@${w.path}/`:`@${w.path}`;$.setText(I)}}function C(m,g){let b=m??g;i.toolBlocks.delete(b),m&&(i.toolIdToName.delete(m),m!==g&&i.toolBlocks.delete(g))}function y(m,g){let b=s.messagesEl.createDiv({cls:"chat-tool-block running"}),k=b.createDiv({cls:"chat-tool-header"});k.createSpan({cls:"chat-tool-icon"}).setText(Dr(m)),k.createSpan({cls:"chat-tool-name"}).setText(m),k.createDiv({cls:"chat-tool-spinner"}),b.createDiv({cls:"chat-tool-terminal"}).createSpan({cls:"chat-tool-cursor",text:"\u2588"});let I=g||m;i.toolBlocks.set(I,b),g&&(i.toolIdToName.set(g,m),g!==m&&i.toolBlocks.set(m,b)),o(!1)}function _(m,g){let b,k=i.toolBlocks.get(m);if(k&&(b=k,C(void 0,m)),!b){for(let[h,w]of i.toolIdToName)if(w===m){b=i.toolBlocks.get(h),C(h,m);break}}if(!b){let h=s.messagesEl.querySelectorAll(".chat-tool-block.running");h.length&&(b=h[h.length-1])}b?c(b,m,g):s.messagesEl.createDiv({cls:"chat-msg status"}).setText(`\u2705 ${m} \u5B8C\u6210`),o(!1)}function A(m,g){let b=s.messagesEl.createDiv({cls:"chat-tool-block done"});b.createDiv({cls:"chat-tool-header"}),b.createDiv({cls:"chat-tool-terminal"}),c(b,m,g),o(!1)}function M(m){let g=fn(m),b=qr(g),k=ha(g),h;if(k?(h=i.toolBlocks.get(k)??i.toolBlocks.get(b),C(k,b)):i.toolBlocks.has(b)&&(h=i.toolBlocks.get(b),C(void 0,b)),!h){let w=s.messagesEl.querySelectorAll(".chat-tool-block.running");w.length&&(h=w[w.length-1])}h?u(h,g):s.messagesEl.createDiv({cls:"chat-msg status"}).setText(`${vn(Yr(g))}: ${b}`),o(!1)}function z(m){let g=fn(m),b=s.messagesEl.createDiv({cls:"chat-tool-block done"});b.createDiv({cls:"chat-tool-header"}),b.createDiv({cls:"chat-tool-terminal"}),u(b,g),o(!1)}function U(){i.toolBlocks.clear(),i.toolIdToName.clear()}function j(){s.messagesEl.querySelectorAll(".chat-msg.status, .chat-tool-block.running").forEach(m=>m.remove())}function X(){i.messages=[],i.userMsgRefs=[],U(),s.messagesEl.empty(),K(),s.minimapEl.querySelectorAll(".chat-minimap-dot").forEach(m=>m.remove())}function K(){let m="\u4E0A\u4E0B\u6587\u7EDF\u8BA1\u4F1A\u5728\u4E0B\u4E00\u6B21\u6A21\u578B\u54CD\u5E94\u5B8C\u6210\u540E\u66F4\u65B0\u3002";s.contextBarEl.style.display="flex",s.contextBarEl.removeAttribute("title"),s.contextBarEl.setAttribute("aria-label",m),(0,Dt.setTooltip)(s.contextBarEl,m,{placement:"top",delay:120,classes:["life-context-tooltip"]}),s.contextBarEl.empty(),s.contextBarEl.createSpan({cls:"context-meter-label",text:"\u4E0A\u4E0B\u6587"});let g=s.contextBarEl.createDiv({cls:"context-ring",attr:{"aria-hidden":"true"}});g.style.setProperty("--context-progress","0%"),g.style.setProperty("--context-color","var(--text-muted)");let b=s.contextBarEl.createSpan({cls:"context-percent-label"});b.style.color="var(--text-muted)",b.setText("0%"),s.contextBarEl.createSpan({cls:"context-separator",text:"\xB7"}),s.contextBarEl.createSpan({cls:"context-usage-label",text:"\u7528\u91CF \u6682\u65E0"})}function S(m){s.contextBarEl.style.display="flex";let g=m.usage_percent,b=Pa(g),k=Math.max(0,Math.min(g,100)),h=m.actual_usage,w=m.cumulative_usage,$=Sa(h,w),I="var(--text-success)";g>80?I="var(--text-error)":g>50&&(I="var(--text-warning, #e0a030)");let V=Ea(m,b);s.contextBarEl.removeAttribute("title"),s.contextBarEl.setAttribute("aria-label",V),(0,Dt.setTooltip)(s.contextBarEl,V,{placement:"top",delay:120,classes:["life-context-tooltip"]}),s.contextBarEl.empty(),s.contextBarEl.createSpan({cls:"context-meter-label",text:"\u4E0A\u4E0B\u6587"});let J=s.contextBarEl.createDiv({cls:"context-ring",attr:{"aria-hidden":"true"}});J.style.setProperty("--context-progress",`${k}%`),J.style.setProperty("--context-color",I);let Z=s.contextBarEl.createSpan({cls:"context-percent-label"});Z.style.color=I,Z.setText(b),s.contextBarEl.createSpan({cls:"context-separator",text:"\xB7"}),s.contextBarEl.createSpan({cls:"context-usage-label",text:`\u7528\u91CF ${$}`})}function D(m){a=m}return K(),{appendMessage:p,renderAssistantMessage:f,beginTool:y,completeTool:M,renderHistoricalTool:z,clearConversationUi:X,clearToolTracking:U,removeTransientUi:j,scrollToBottom:o,updateContextBar:S,updateLastUserMessageId:x,setForkHandler:D}}var Jr=require("obsidian");var Ma="\uFF08\u7CFB\u7EDF\u901A\u77E5\uFF1A\u4E0A\u6B21\u6295\u9012\u5230\u540E\u53F0\u7684\u4EFB\u52A1\u521A\u521A\u5B8C\u6210\uFF0C\u8BF7\u76F4\u63A5\u6839\u636E\u65B0\u6CE8\u5165\u7684 <task_notification> \u4E0A\u4E0B\u6587\u7EE7\u7EED\u56DE\u590D\u6211\u3002\uFF09";function Xr(t){let{client:e,composer:n,elements:r,state:s,transcript:i,sessions:a,persona:l,plugin:o,diaryPrompt:c}=t;function u(P){if(r.inputEl.disabled=P,r.attachmentBtn.disabled=P,P){r.sendBtn.classList.add("is-stop"),r.sendBtn.innerHTML=wr,r.sendBtn.setAttribute("aria-label","\u505C\u6B62");return}r.sendBtn.classList.remove("is-stop"),r.sendBtn.innerHTML=Tt,r.sendBtn.setAttribute("aria-label","\u53D1\u9001")}async function p(P,C){let y=r.messagesEl.createDiv({cls:"chat-msg assistant"});y.setText("\u601D\u8003\u4E2D..."),i.scrollToBottom();try{let _=await e.chat(P.request);y.remove(),_.warnings?.forEach(M=>i.appendMessage("status",M)),l.setPersonaState(_.persona_state),C&&i.updateLastUserMessageId(_.user_message_id??void 0),_.tool_calls?.forEach(M=>{i.renderHistoricalTool(M)});let A=Da(_.tool_calls??[]);i.appendMessage("assistant",_.reply,!0,[],_.message_id??void 0),_.context&&i.updateContextBar(_.context),await a.syncCurrentSessionTitle(_.session_id),A&&c.showLoopStopResult(A,_.session_id,_.conversation_id)}catch(_){y.remove();let A=_ instanceof Error?_.message:String(_);i.appendMessage("assistant",`\u274C \u8FDE\u63A5\u51FA\u9519: ${A}

\u8BF7\u68C0\u67E5\u540E\u7AEF\u662F\u5426\u53EF\u8BBF\u95EE\uFF0C\u6216\u67E5\u770B\u540E\u7AEF\u65E5\u5FD7\u3002`)}}async function f(P){let C=P?{request:{content:P,persona_mode:s.personaState.mode,manual_persona_id:s.personaState.manual_persona_id},displayText:P,displayAttachments:[]}:(()=>{let h=n.getSubmitPayload();return h?(h.request.persona_mode=s.personaState.mode,h.request.manual_persona_id=s.personaState.manual_persona_id,h):null})();if(!C||s.isSending)return;c.hide();let y=!P,_=await o.applyLlmProfile();if(!_.ok){i.appendMessage("assistant",`\u274C ${_.message}

\u8BF7\u5728\u8BBE\u7F6E\u4E2D\u914D\u7F6E LLM \u540E\u518D\u8BD5\u3002`);return}let A=await o.ensureBackendVaultPathSynced(e);A.ok||i.appendMessage("status",`Warning: failed to sync the current vault path before sending. ${A.message}`,!1),s.isSending=!0,s.isAborted=!1,u(!0),P||n.clear(),P?i.appendMessage("status","[\u7CFB\u7EDF\u4EE3\u7406\u81EA\u52A8\u89E6\u53D1\uFF1A\u68C0\u67E5\u7CFB\u7EDF\u901A\u77E5]"):i.appendMessage("user",C.displayText,!0,C.displayAttachments);let M=null,z="",U="",j="",X=null,K=null,S=null,D=()=>Mt(U,z),m=()=>{let h=D();if(j=h,!h&&!M)return;M||(M=r.messagesEl.createDiv({cls:"chat-msg assistant streaming"}));let w=U.trim();X||(X=Fr(M)),X.render(z,w),i.scrollToBottom(!1)},g=()=>{j=D(),K===null&&(K=requestAnimationFrame(()=>{K=null,m()}))},b=()=>{K!==null&&(cancelAnimationFrame(K),K=null),m()},k=()=>{K!==null&&(cancelAnimationFrame(K),K=null)};try{await e.streamChat(C.request,{onAssistantPrefix:h=>{z+=h,g()},onReasoningDelta:h=>{U+=h,g()},onTextDelta:h=>{z+=h,g()},onToolStart:(h,w)=>{(M||D().trim())&&b();let $=D();if(M&&$.trim()){let I=yn(M);M.empty(),M.classList.remove("streaming"),i.renderAssistantMessage(M,$),kn(M,I)}else M&&M.remove();z="",U="",j="",X=null,M=null,i.beginTool(h,w)},onToolResult:h=>{i.completeTool(h),Zr(h)&&(S=h)},onWarning:h=>{i.appendMessage("status",h,!1)},onDone:async(h,w,$,I,V,J)=>{if(!s.isAborted){if(y&&i.updateLastUserMessageId(I),(M||D().trim())&&b(),M){M.classList.remove("streaming");let Z=D();if(Z.trim()){let te=yn(M);M.empty(),i.renderAssistantMessage(M,Z,$),kn(M,te),X=null}else M.childNodes.length||M.remove()}s.messages.push({role:"assistant",content:j,messageId:$}),V&&i.updateContextBar(V),J&&l.setPersonaState(J),S&&(c.showLoopStopResult(S,h,w),S=null),await a.syncCurrentSessionTitle(h)}},onError:h=>{let w=h.message;s.isAborted||((M||D().trim())&&b(),M&&!D()&&M.remove(),i.appendMessage("assistant",`\u274C \u51FA\u9519: ${w}

\u8BF7\u68C0\u67E5\u540E\u7AEF\u662F\u5426\u53EF\u8BBF\u95EE\uFF0C\u6216\u67E5\u770B\u540E\u7AEF\u65E5\u5FD7\u3002`))}})}catch(h){if(!s.isAborted){(M||D().trim())&&b();let w=M;if(w){let $=D();if($.trim()){let I=yn(w);w.classList.remove("streaming"),w.empty(),i.renderAssistantMessage(w,$),kn(w,I),X=null}else w.remove()}i.removeTransientUi(),i.clearToolTracking(),Wn(h)&&await p(C,y)}}finally{if(s.isAborted){(M||D().trim())&&b();let h=M;if(h)if(h.classList.remove("streaming"),D()){let w=document.createElement("span");w.className="abort-hint",w.textContent=" [\u5DF2\u4E2D\u6B62]",h.appendChild(w)}else h.remove();j&&s.messages.push({role:"assistant",content:j}),i.removeTransientUi(),i.clearToolTracking()}k(),s.isAborted=!1,s.isSending=!1,u(!1)}}function x(){s.isAborted=!0,e.abort()}function d(P){i.appendMessage("status",P.message),new Jr.Notice("\u540E\u53F0\u4EFB\u52A1\u6709\u65B0\u7684\u5B8C\u6210\u901A\u77E5\u3002"),P.autoTrigger&&!s.isSending&&f(Ma)}return{handleSend:f,handleStop:x,handleSysNotify:d}}function yn(t){return!!t.querySelector(".chat-thought-block.expanded")}function kn(t,e){if(!e)return;let n=t.querySelector(".chat-thought-block"),r=t.querySelector(".chat-thought-header"),s=t.querySelector(".chat-thought-chevron");n?.classList.add("expanded"),r?.setAttribute("aria-expanded","true"),s&&s.setText("v")}function Da(t){for(let e=t.length-1;e>=0;e-=1){let n=t[e];if(Zr(n))return n}return null}function Zr(t){let e=t.name||t.tool||"",n=t.metadata?.job_id;return e==="loop_stop"&&!t.is_error&&t.status!=="error"&&typeof n=="string"&&n.trim().length>0}var Qe="crabby-chat",Rt=class extends At.ItemView{constructor(n,r){super(n);this.plugin=r;this.state={messages:[],userMsgRefs:[],toolBlocks:new Map,toolIdToName:new Map,isSending:!1,isAborted:!1,sessionPanelOpen:!1,treePanelOpen:!1,personaState:Le()};this.cleanupFns=[];this.client=new W(this.plugin.settings.backendUrl)}getViewType(){return Qe}getDisplayText(){return"Crabby"}getIcon(){return"bot"}async onOpen(){this.cleanupFns=[],this.state.messages=[],this.state.userMsgRefs=[],this.state.toolBlocks.clear(),this.state.toolIdToName.clear(),this.state.isSending=!1,this.state.isAborted=!1,this.state.sessionPanelOpen=!1,this.state.treePanelOpen=!1,this.state.personaState=Le();let n=this.contentEl;n.empty(),n.addClass("crabby-chat");let r=n.createDiv({cls:"chat-header-area"}),s=r.createDiv({cls:"chat-header-actions chat-header-actions-left"}),i=s.createEl("button",{cls:"chat-header-btn chat-history-btn",attr:{"aria-label":"\u5386\u53F2\u4F1A\u8BDD"}});i.innerHTML=Sr;let a=s.createEl("button",{cls:"chat-header-btn chat-tree-btn",attr:{"aria-label":"\u4F1A\u8BDD\u6811"}});a.innerHTML=Er;let l=r.createDiv({cls:"chat-header-title"});l.setText("\u65B0\u4F1A\u8BDD");let c=r.createDiv({cls:"chat-header-actions chat-header-actions-right"}).createEl("button",{cls:"chat-header-btn chat-new-btn",attr:{"aria-label":"\u65B0\u5EFA\u4F1A\u8BDD"}});c.innerHTML=_r;let u=n.createDiv({cls:"session-panel"}),p=u.createDiv({cls:"session-panel-header"});p.createEl("span",{text:"\u5386\u53F2\u4F1A\u8BDD",cls:"session-panel-title"});let f=p.createEl("button",{cls:"session-panel-close",attr:{"aria-label":"\u5173\u95ED"}});f.setText("\xD7");let x=u.createDiv({cls:"session-list"}),d=n.createDiv({cls:"session-panel tree-panel"}),P=d.createDiv({cls:"session-panel-header"}),C=P.createSpan({cls:"session-panel-title"});C.setText("\u4F1A\u8BDD\u6811");let y=P.createEl("button",{cls:"session-panel-close",attr:{"aria-label":"\u5173\u95ED\u4F1A\u8BDD\u6811"}});y.setText("\xD7");let _=d.createDiv({cls:"conversation-tree-list"}),A=n.createDiv({cls:"chat-body"});if(!this.plugin.settings.llmProfiles.some(N=>!ve(N))){let N=A.createDiv({cls:"chat-no-profile-banner"});N.createDiv({cls:"chat-no-profile-banner-icon"}).setText("!"),N.createDiv({cls:"chat-no-profile-banner-text"}).createSpan({text:"\u5C1A\u672A\u914D\u7F6E LLM\uFF0C\u5F53\u524D\u65E0\u6CD5\u53D1\u9001\u6D88\u606F\u3002"}),N.createEl("button",{cls:"chat-no-profile-banner-btn",text:"\u524D\u5F80\u8BBE\u7F6E"}).addEventListener("click",()=>{N.remove(),this.openPluginSettings()||new At.Notice("\u65E0\u6CD5\u81EA\u52A8\u6253\u5F00 Crabby \u8BBE\u7F6E\uFF0C\u8BF7\u4ECE Obsidian \u8BBE\u7F6E\u4E2D\u6253\u5F00\u63D2\u4EF6\u8BBE\u7F6E\u3002")})}let z=A.createDiv({cls:"chat-minimap"});z.createDiv({cls:"chat-minimap-line"});let U=A.createDiv({cls:"chat-messages"}),j=n.createDiv({cls:"chat-footer"}),X=j.createDiv({cls:"chat-diary-prompt"}),K=j.createDiv({cls:"chat-input-area"}),S=K.createDiv({cls:"chat-composer-pills"}),D=K.createDiv({cls:"chat-suggestion-list"}),m=K.createDiv({cls:"chat-input-row"}),g=m.createEl("button",{cls:"chat-attach-btn",attr:{"aria-label":"\u9009\u62E9\u56FE\u7247"}});g.innerHTML=Cr;let b=m.createEl("textarea",{cls:"chat-input",attr:{placeholder:"\u8F93\u5165\u6D88\u606F\uFF0C\u652F\u6301 /skill\u3001@\u6587\u4EF6 \u548C\u7C98\u8D34\u56FE\u7247...",rows:"1"}}),k=m.createEl("button",{cls:"chat-send-btn",attr:{"aria-label":"\u53D1\u9001"}});k.innerHTML=Tt;let h=m.createEl("input",{attr:{type:"file",accept:"image/*",multiple:"true"}});h.addClass("chat-hidden-file-input");let w=j.createDiv({cls:"chat-model-area"}),$=w.createDiv({cls:"chat-context-bar"});this.elements={messagesEl:U,minimapEl:z,diaryPromptEl:X,inputAreaEl:K,inputEl:b,sendBtn:k,attachmentBtn:g,hiddenFileInput:h,composerPillsEl:S,suggestionListEl:D,contextBarEl:$,sessionTitleEl:l,sessionPanelEl:u,sessionListEl:x,treePanelEl:d,treePanelTitleEl:C,treeListEl:_},Kr();let I=kr({app:this.app,component:this,client:this.client,plugin:this.plugin,elements:this.elements,state:this.state});this.cleanupFns.push(()=>I.destroy());let V=Gr({app:this.app,component:this,client:this.client,plugin:this.plugin,elements:this.elements,state:this.state}),J=Rr(w,this.client,this.state);this.cleanupFns.push(()=>J.destroy());let Z=zr({app:this.app,component:this,client:this.client,plugin:this.plugin,elements:this.elements,state:this.state,composer:I,transcript:V,persona:J}),te=Pr({app:this.app,client:this.client,plugin:this.plugin,rootEl:X,openPluginSettings:()=>this.openPluginSettings()});this.cleanupFns.push(()=>te.destroy());let ee=Xr({app:this.app,component:this,client:this.client,plugin:this.plugin,elements:this.elements,state:this.state,composer:I,transcript:V,sessions:Z,persona:J,diaryPrompt:te});this.cleanupFns.push(Ar(w,this.plugin,this.client)),this.client.onSysNotify=N=>{ee.handleSysNotify(N)},this.cleanupFns.push(()=>{this.client.onSysNotify=void 0});let O=()=>{this.client.setBaseUrl(this.plugin.settings.backendUrl)};document.addEventListener(Oe,O),this.cleanupFns.push(()=>{document.removeEventListener(Oe,O)}),i.addEventListener("click",()=>{Z.toggleSessionPanel()}),a.addEventListener("click",()=>{Z.toggleTreePanel()}),f.addEventListener("click",()=>{Z.toggleSessionPanel()}),y.addEventListener("click",()=>{Z.toggleTreePanel()}),c.addEventListener("click",()=>{Z.handleNewSession()}),k.addEventListener("click",()=>{this.state.isSending?ee.handleStop():ee.handleSend()}),b.addEventListener("keydown",N=>{if(!N.defaultPrevented){if(!N.shiftKey&&!N.altKey&&!N.ctrlKey&&!N.metaKey&&(N.key==="ArrowUp"||N.key==="ArrowDown")&&I.navigateHistory(N.key==="ArrowUp"?"up":"down")){N.preventDefault();return}N.key==="Enter"&&!N.shiftKey&&(N.preventDefault(),ee.handleSend())}}),V.appendMessage("assistant","\u4F60\u597D\uFF01\u6211\u662F\u4F60\u7684 Crabby\uFF0C\u6709\u4EC0\u4E48\u53EF\u4EE5\u5E2E\u4F60\u7684\uFF1F")}openPluginSettings(){let n=this.app.setting;return!n?.open&&!n?.openTabById?!1:(n.open?.(),n.openTabById?.(this.plugin.manifest.id),window.setTimeout(()=>n.openTabById?.(this.plugin.manifest.id),0),!0)}async onClose(){for(let n of this.cleanupFns.splice(0).reverse())try{n()}catch{}this.client.disconnect(),this.contentEl.empty()}};var Ps=require("node:fs"),Nt=require("node:path");var Bt=require("node:child_process"),H=require("node:fs"),ys=require("node:net"),B=require("node:path"),$t=require("node:crypto"),lt=require("obsidian");var we=require("node:fs"),et=require("node:path"),es={"identity.md":`\u4F60\u662F Crabby\uFF0C\u8FD0\u884C\u5728\u7528\u6237\u672C\u5730 Obsidian Vault \u91CC\u7684\u7B2C\u4E8C\u5927\u8111\u52A9\u624B\u3002
...
{{entries}}
`};function En(t){if(!lt.Platform.isDesktopApp)throw new Error("Crabby \u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u9700\u8981 Obsidian \u684C\u9762\u7248\u3002");let e=t.vault.adapter;if(!(e instanceof lt.FileSystemAdapter))throw new Error("\u65E0\u6CD5\u89E3\u6790\u684C\u9762\u7AEF vault \u6587\u4EF6\u7CFB\u7EDF\u8DEF\u5F84\u3002");let n=e.getBasePath(),r=(0,B.join)(n,t.vault.configDir,"plugins",Ia),s=(0,B.join)(n,".crabby"),i=(0,B.join)(s,"config"),a=(0,B.join)(s,"data"),l=(0,B.join)(s,"logs"),o=(0,B.join)(s,"memory"),c=(0,B.join)(s,"templates"),u=(0,B.join)(r,"runtime");return{pluginDir:r,userDataDir:s,configDir:i,envPath:(0,B.join)(i,".env"),mcpConfigPath:(0,B.join)(i,"mcp_servers.json"),promptsDir:(0,B.join)(i,"prompts"),personasDir:(0,B.join)(i,"personas"),memoryDir:o,templatesDir:c,dataDir:a,sessionsDir:(0,B.join)(a,"sessions"),attachmentsDir:(0,B.join)(a,"attachments"),logsDir:l,runtimeDir:u,statePath:(0,B.join)(u,"state.json"),heartbeatPath:(0,B.join)(u,"host-heartbeat.json"),devRuntimePath:(0,B.join)(r,".dev-runtime.json")}}var It=class{constructor(e,n){this.app=e;this.settings=n;this.child=null;this.externalBackend=null;this.heartbeatTimer=null;this.statusDetail="\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u5C1A\u672A\u542F\u52A8\u3002";this.layout=En(e)}getLayout(){return this.layout}async ensureRuntimeLayout(){this.migrateLegacyRuntimeData();for(let i of[this.layout.userDataDir,this.layout.configDir,this.layout.promptsDir,this.layout.personasDir,this.layout.memoryDir,this.layout.templatesDir,this.layout.sessionsDir,this.layout.attachmentsDir,this.layout.logsDir,this.layout.runtimeDir,(0,B.dirname)(this.layout.statePath)])(0,H.mkdirSync)(i,{recursive:!0});this.ensureMemoryLayout();let e=this.syncDiaryConfig();e.ok||this.appendRuntimeLog(`failed to sync diary config: ${e.message}`);let n=this.ensureAdminToken();Fe(this.layout.envPath,{CRABBY_ADMIN_ENABLED:"true",CRABBY_ADMIN_TOKEN:n,...this.getHostWatchdogEnv(),CRABBY_BACKEND_RELOADER_PARENT:"false",VAULT_PATH:this.getVaultBasePath(),HOST:Te,PROMPTS_DIR:this.layout.promptsDir,PERSONAS_DIR:this.layout.personasDir}),this.startHostHeartbeat();let r=ts(this.layout.promptsDir,es),s=ns(this.layout.personasDir);return r&&this.appendRuntimeLog("seeded default prompt templates"),s.seeded&&this.appendRuntimeLog("seeded default persona templates"),s.migrated&&this.appendRuntimeLog("migrated legacy default persona templates"),(0,H.existsSync)(this.layout.mcpConfigPath)||(0,H.writeFileSync)(this.layout.mcpConfigPath,`${JSON.stringify({mcpServers:{}},null,2)}
`,"utf8"),this.settings.backendEnvPath=this.layout.envPath,this.settings.backendMcpConfigPath=this.layout.mcpConfigPath,this.settings.backendPath="",this.appendRuntimeLog("runtime layout ensured"),this.layout}async start(){if(await this.ensureRuntimeLayout(),this.appendRuntimeLog("start requested"),this.child&&!this.child.killed)return this.appendRuntimeLog(`start skipped because child is already running: pid=${this.child.pid??"unknown"}`),this.getStatus();if(this.externalBackend){let f=this.ensureAdminToken();if(await wn(this.externalBackend.backendUrl,f))return this.appendRuntimeLog(`start skipped because existing backend is reachable: ${this.externalBackend.backendUrl}`),this.getStatus();this.appendRuntimeLog(`discarding unreachable existing backend: ${this.externalBackend.backendUrl}`),this.externalBackend=null}let e=this.resolveLaunchConfig();if(!e)return this.statusDetail="\u6B63\u5F0F\u7248\u540E\u7AEF\u7A0B\u5E8F\u5C1A\u672A\u5B89\u88C5\u3002",this.appendRuntimeLog("start aborted: no launch config"),this.getStatus();let n=await this.reuseExistingBackendIfAvailable(e);if(n)return n;let r=await ja(ms),s=`http://${Te}:${r}`,i=e.mode==="dev"?hs(e.args,Te,r):e.args,a=fs(i);this.appendRuntimeLog(`launch config resolved: mode=${e.mode} command=${e.command} args=${JSON.stringify(e.args)} cwd=${e.cwd} port=${r}`);let l=this.ensureAdminToken();Fe(this.layout.envPath,{CRABBY_ADMIN_ENABLED:"true",CRABBY_ADMIN_TOKEN:l,...this.getHostWatchdogEnv(),CRABBY_BACKEND_RELOADER_PARENT:a,VAULT_PATH:this.getVaultBasePath(),HOST:Te,PORT:String(r),PROMPTS_DIR:this.layout.promptsDir,PERSONAS_DIR:this.layout.personasDir});let o=(0,H.createWriteStream)((0,B.join)(this.layout.logsDir,"backend-out.log"),{flags:"a"}),c=(0,H.createWriteStream)((0,B.join)(this.layout.logsDir,"backend-error.log"),{flags:"a"}),u={...process.env,VAULT_PATH:this.getVaultBasePath(),MCP_CONFIG_FILE:this.layout.mcpConfigPath,DATA_DIR:this.layout.dataDir,LOG_DIR:this.layout.logsDir,...this.getHostWatchdogEnv(),CRABBY_BACKEND_RELOADER_PARENT:a,HOST:Te,PORT:String(r),PROMPTS_DIR:this.layout.promptsDir,PERSONAS_DIR:this.layout.personasDir,PYTHONUNBUFFERED:"1",PYTHONIOENCODING:"utf-8"},p=qa(u);u[p]=Ya(u[p]),this.appendRuntimeLog(`spawning backend: ${e.command} ${i.join(" ")}`);try{this.child=(0,Bt.spawn)(e.command,i,{cwd:e.cwd,env:u,windowsHide:!0})}catch(f){let x=f instanceof Error?f.message:String(f);return this.statusDetail=`\u540E\u7AEF\u8FDB\u7A0B\u542F\u52A8\u5931\u8D25\uFF1A${x}`,this.appendRuntimeLog(`spawn threw synchronously: ${x}`),o.end(),c.end(),this.getStatus()}this.child.stdout.pipe(o),this.child.stderr.pipe(c),this.child.once("error",f=>{this.statusDetail=`\u540E\u7AEF\u8FDB\u7A0B\u542F\u52A8\u5931\u8D25\uFF1A${f.message}`,this.appendRuntimeLog(`child error: ${f.message}`),this.child=null,o.end(),c.end()}),this.child.once("exit",(f,x)=>{this.statusDetail=`\u540E\u7AEF\u8FDB\u7A0B\u5DF2\u9000\u51FA\uFF0C\u9000\u51FA\u7801 ${f??"null"}\uFF0C\u4FE1\u53F7 ${x??"null"}\u3002`,this.appendRuntimeLog(`child exited: code=${f??"null"} signal=${x??"null"}`),this.child=null,o.end(),c.end()}),this.settings.backendUrl=s,this.writeState({mode:e.mode,version:e.version,platform:process.platform,executablePath:e.command,port:r,pid:this.child.pid,startedAt:new Date().toISOString()});try{await Ga(s,Ba),this.statusDetail=`\u540E\u7AEF\u6B63\u5728\u4EE5${e.mode==="dev"?"\u5F00\u53D1\u7248":"\u6B63\u5F0F\u7248"}\u8FD0\u884C\u3002`,this.appendRuntimeLog(`health check passed: ${s}`)}catch(f){this.statusDetail=f instanceof Error?f.message:"\u540E\u7AEF\u5065\u5EB7\u68C0\u67E5\u5931\u8D25\u3002",this.appendRuntimeLog(`health check failed: ${this.statusDetail}`)}return this.getStatus()}async stop(){this.stopHostHeartbeat();let e=this.child;if(!e||e.killed)return this.stopExistingBackendWithoutChild();let n=this.ensureAdminToken(),r=this.settings.backendUrl;try{await gs(r,n),await ks(e,ps)}catch{await Xa(e)}return this.child=null,this.statusDetail="\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u5DF2\u505C\u6B62\u3002",this.getStatus()}async restart(){return await this.stop(),this.start()}async installRuntime(e){await this.ensureRuntimeLayout();let n=e.trim();if(!n)throw new Error("\u5C1A\u672A\u914D\u7F6E\u540E\u7AEF\u7A0B\u5E8F\u4E0B\u8F7D\u6E05\u5355 URL\u3002");let r=await fetch(n);if(!r.ok)throw new Error(`\u540E\u7AEF\u7A0B\u5E8F\u4E0B\u8F7D\u6E05\u5355\u83B7\u53D6\u5931\u8D25\uFF1AHTTP ${r.status}`);let s=await r.json(),i=s.platforms?.[process.platform];if(!i)throw new Error(`\u5F53\u524D\u5E73\u53F0\u6CA1\u6709\u53EF\u7528\u7684\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\uFF1A${process.platform}\u3002`);let a=await fetch(i.url);if(!a.ok)throw new Error(`\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u4E0B\u8F7D\u5931\u8D25\uFF1AHTTP ${a.status}`);let l=Buffer.from(await a.arrayBuffer());if((0,$t.createHash)("sha256").update(l).digest("hex").toLowerCase()!==i.sha256.toLowerCase())throw new Error("\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F SHA256 \u6821\u9A8C\u5931\u8D25\u3002");let c=i.executableName??(process.platform==="win32"?"crabby-backend.exe":"crabby-backend"),u=(0,B.join)(this.layout.runtimeDir,"backend",s.version,process.platform);(0,H.mkdirSync)(u,{recursive:!0});let p=(0,B.join)(u,c);return(0,H.writeFileSync)(p,l),process.platform!=="win32"&&(0,H.chmodSync)(p,493),this.writeState({mode:"production",version:s.version,platform:process.platform,executablePath:p}),this.statusDetail=`\u5DF2\u5B89\u88C5\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F ${s.version}\u3002`,this.getStatus()}getStatus(){let e=this.readState(),n=this.readDevRuntimeConfig(),r=n?"dev":"production",s=this.externalBackend?.port??bs(this.settings.backendUrl)??e?.port??null,i=!!(this.child&&!this.child.killed)||!!this.externalBackend,a=r==="dev"?e?.version?.trim()||"dev":e?.version?.trim()||"-";return{mode:r,version:a,installed:!!(n||e?.executablePath),running:i,backendUrl:s!==null?`http://${Te}:${s}`:this.settings.backendUrl,port:s,pid:i?this.child?.pid??this.externalBackend?.pid??null:null,envPath:this.layout.envPath,mcpConfigPath:this.layout.mcpConfigPath,promptsDir:this.layout.promptsDir,personasDir:this.layout.personasDir,memoryDir:this.layout.memoryDir,templatesDir:this.layout.templatesDir,dataDir:this.layout.dataDir,logsDir:this.layout.logsDir,detail:this.statusDetail}}resolveLaunchConfig(){let e=this.readDevRuntimeConfig();if(e)return{mode:"dev",command:e.backendCommand,args:e.backendArgs,cwd:e.backendCwd};let n=this.readState(),r=n?.mode==="production"?us(this.layout.runtimeDir,n.executablePath):null;return n?.mode==="production"&&r&&(0,H.existsSync)(r)?{mode:"production",command:r,args:[],cwd:(0,B.dirname)(r),version:n.version}:null}async reuseExistingBackendIfAvailable(e){let n=this.ensureAdminToken(),r=await this.findExistingManagedBackend(n);if(!r)return null;this.externalBackend=r,this.settings.backendUrl=r.backendUrl,this.startHostHeartbeat();let s=e.mode==="dev"?hs(e.args,Te,r.port):e.args;return Fe(this.layout.envPath,{CRABBY_ADMIN_ENABLED:"true",CRABBY_ADMIN_TOKEN:n,...this.getHostWatchdogEnv(),CRABBY_BACKEND_RELOADER_PARENT:fs(s),VAULT_PATH:this.getVaultBasePath(),HOST:Te,PORT:String(r.port),PROMPTS_DIR:this.layout.promptsDir,PERSONAS_DIR:this.layout.personasDir}),this.writeState({mode:e.mode,version:e.version,platform:process.platform,executablePath:e.command,port:r.port,pid:r.pid??void 0,startedAt:new Date().toISOString()}),this.statusDetail="Backend already running; reusing existing managed process.",this.appendRuntimeLog(`reusing existing backend: ${r.backendUrl} pid=${r.pid??"unknown"}`),this.getStatus()}async stopExistingBackendWithoutChild(){this.child=null;let e=this.ensureAdminToken(),n=this.externalBackend??await this.findExistingManagedBackend(e);if(!n)return this.externalBackend=null,this.statusDetail="\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u5F53\u524D\u672A\u8FD0\u884C\u3002",this.getStatus();try{await gs(n.backendUrl,e),await Ja(n.backendUrl,ps),this.appendRuntimeLog(`shutdown requested for existing backend: ${n.backendUrl}`)}catch(r){let s=r instanceof Error?r.message:String(r);if(this.appendRuntimeLog(`failed to stop existing backend ${n.backendUrl}: ${s}`),await wn(n.backendUrl,e))return this.externalBackend=n,this.statusDetail=`Backend shutdown failed: ${s}`,this.getStatus()}return this.externalBackend=null,this.statusDetail="\u672C\u5730\u540E\u7AEF\u7A0B\u5E8F\u5DF2\u505C\u6B62\u3002",this.getStatus()}async findExistingManagedBackend(e){let n=this.readState();for(let r of Va([bs(this.settings.backendUrl),n?.port??null,ms])){let s=`http://${Te}:${r}`;if(await wn(s,e))return{backendUrl:s,port:r,pid:n?.port===r?n.pid??null:null}}return null}readDevRuntimeConfig(){if(!(0,H.existsSync)(this.layout.devRuntimePath))return null;try{let e=JSON.parse(vs((0,H.readFileSync)(this.layout.devRuntimePath,"utf8")));if(e?.mode==="dev"&&typeof e.backendCommand=="string"&&Array.isArray(e.backendArgs)&&typeof e.backendCwd=="string")return{mode:"dev",repoRoot:(0,B.resolve)(String(e.repoRoot??"")),backendCommand:(0,B.resolve)(e.backendCommand),backendArgs:e.backendArgs.map(String),backendCwd:(0,B.resolve)(e.backendCwd)}}catch{return null}return null}readState(){if(!(0,H.existsSync)(this.layout.statePath))return null;try{return JSON.parse(vs((0,H.readFileSync)(this.layout.statePath,"utf8")))}catch{return null}}writeState(e){(0,H.mkdirSync)((0,B.dirname)(this.layout.statePath),{recursive:!0});let n=this.normalizeRuntimeStateForWrite(e);(0,H.writeFileSync)(this.layout.statePath,`${JSON.stringify(n,null,2)}
`,"utf8")}normalizeRuntimeStateForWrite(e){return e.mode!=="production"||!e.executablePath?e:{...e,executablePath:ds(this.layout.runtimeDir,e.executablePath)}}migrateLegacyRuntimeData(){let e=this.layout.pluginDir,n=[{label:"config",legacyPath:(0,B.join)(e,"config"),targetPath:this.layout.configDir},{label:"data",legacyPath:(0,B.join)(e,"data"),targetPath:this.layout.dataDir},{label:"logs",legacyPath:(0,B.join)(e,"logs"),targetPath:this.layout.logsDir}];for(let r of is(n))r.status!=="missing"&&this.appendRuntimeLog([`legacy ${r.label} migration: ${r.status}`,`from=${r.legacyPath}`,`to=${r.targetPath}`,`moved=${r.movedEntries}`,`skipped=${r.skippedEntries}`,`message=${r.message}`].join(" "))}appendRuntimeLog(e){try{(0,H.mkdirSync)(this.layout.logsDir,{recursive:!0}),(0,H.appendFileSync)((0,B.join)(this.layout.logsDir,"runtime-manager.log"),`${new Date().toISOString()} ${e}
