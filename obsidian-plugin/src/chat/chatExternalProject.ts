/**
 * External project modal: register an external code directory for the current
...
const ACCESS_LEVEL_LABELS: Record<string, string> = {
  "read-only": "只读（可读外部项目，写入仅限 Vault）",
  "workspace-write": "可写（读写外部项目，bash 在项目目录执行）",
...
class ExternalProjectModal extends Modal {
  ...
  private async render(): Promise<void> {
    ...
    contentEl.createEl("h2", { text: "外部项目" });
    contentEl.createEl("p", {
      cls: "external-project-hint",
      text:
        "为当前会话注册一个外部代码目录，Crabby 即可访问 Vault 与该目录。" +
        "Vault 用于存放规划、理解与实现记录，外部目录是实际代码。",
    });

    // Load current session state + available access levels in parallel.
    try {
  ...
  private renderCurrentState(parent: HTMLElement): void {
    ...
    if (this.pathValue) {
      current.createEl("div", {
        text: `当前外部项目：${this.pathValue}`,
      });
      current.createEl("div", {
        text: `访问等级：${accessLevelLabel(this.levelValue)}`,
      });
    } else {
      current.createEl("div", { text: "当前会话未注册外部项目（纯 Vault 对话）。" });
    }
  }
  ...
  private renderPathInput(parent: HTMLElement): void {
    new Setting(parent)
      .setName("外部项目目录")
      .setDesc("绝对路径，例如 D:\\code\\my-app。留空并应用可解除绑定。")
      .addText((text) => {
  ...
  private renderLevelSelect(parent: HTMLElement): void {
    new Setting(parent)
      .setName("访问等级")
      .setDesc("控制对外部项目的写入与 shell 能力；Vault 始终可读写。")
  ...
  private async renderBindings(parent: HTMLElement): Promise<void> {
    ...
    section.createEl("h3", { text: "已保存的项目绑定" });
    section.createEl("p", {
      cls: "external-project-hint",
      text:
        "绑定把 Vault 内的规划目录映射到外部代码目录，便于快速复用。" +
        "点击某条绑定可填入上方路径。",
    });
  ...
  private async applyClear(): Promise<void> {
    ...
    try {
      ...
      new Notice("已解除外部项目，恢复纯 Vault 对话。");
