...
export function getSearchableFiles(app: App): TFile[] {
  const markdownFiles = app.vault.getMarkdownFiles();
  const canvasFiles = app.vault
    .getFiles()
    .filter((file) => getExt(file) === "canvas");
  return [...markdownFiles, ...canvasFiles].filter(
    (file) => !isBlockedPath(file.path),
  );
}

export async function buildDocumentForFile(
  app: App,
  file: TFile,
): Promise<SearchDocument | null> {
  if (isBlockedPath(file.path)) {
    return null;
  }
  try {
    ...
    const document =
      getExt(file) === "canvas"
        ? buildCanvasDocument(file, content)
  ...
  try {
    ...
    const blocks = (parsed.nodes ?? [])
      .map((node) => {
        ...
        if (type === "text") return String(node.text ?? "").trim();
        if (type === "file") return String(node.file ?? "").trim();
        if (type === "link") return String(node.url ?? "").trim();
        if (type === "group") return String(node.label ?? "").trim();
        return "";
      })
      .filter(Boolean)
      .map((text) => ({ text }));
...
export function getExt(file: TFile): string {
