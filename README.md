...
cd server
uv sync --dev
uv run --with pyinstaller python ../scripts/build-backend-runtime.py --version 0.4.0
cd ..
```
...
```text
dist/obsidian-plugin/crabby-0.4.0-darwin-arm64.zip
```
...
```bash
chmod +x "<YourVault>/.obsidian/plugins/crabby/runtime/backend/0.4.0/darwin/crabby-backend"
```
